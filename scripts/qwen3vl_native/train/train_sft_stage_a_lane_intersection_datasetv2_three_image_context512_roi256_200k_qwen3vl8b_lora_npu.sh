...
# ============================================================
# DI/NPU LoRA comparison experiment B.
# Native Qwen3-VL-8B consumes the same matched 200k three-image
# context512/ROI256 records as the DINOv2 + derived-LLM experiment A.
# ============================================================
...
echo "[di-entry] reached matched-200k three-image native-Qwen3VL-8B LoRA launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_three_image_context512_roi256_200k_native_qwen3vl8b_lora_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:?DATASET_OBS_PATH is required}                # Exact three-image context512/ROI256 800k TAR URI.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-rawlane_pose_three_image_context512_roi256_800k}  # Expected root after extraction.
EXPECTED_DATASET_VARIANT=${EXPECTED_DATASET_VARIANT:-rawlane_pose_three_image_context512_roi256_800k}
...
MERGER_LORA_LR=${MERGER_LORA_LR:-${VISION_LORA_LR}}                              # Keep merger LoRA at the visual-side LoRA LR.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_three_image_context512_roi256_200k_ab}  # Shared A/B group.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-b_native_qwen3vl8b_lora_three_image_200k}  # Experiment B.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,ab_test,phase_a,lane_intersection,datasetv2,three_image,context512_roi256,200k,native_qwen3vl8b}  # Tags.
...
python - "${DATASET_PATH}" "${EXPECTED_DATASET_VARIANT}" "${EXPECTED_SOURCE_TRAIN_SAMPLES}" "${TRAIN_SAMPLE_LIMIT}" <<'PY'
import json
import sys
from pathlib import Path
...
if actual_variant != expected_variant:
    raise SystemExit(f"Expected dataset variant {expected_variant!r}, found {actual_variant!r}")
multi = info.get("multi_image_input") or {}
expected_roles = ["bev_road_structure", "pv_camera_raw_lane", "historical_vehicle_trajectory"]
if int(multi.get("num_images_per_sample", 0)) != 3 or list(multi.get("image_order") or []) != expected_roles:
...
print(f"[three-image-preflight] variant={actual_variant} source={records} selected={sample_limit}")
PY
