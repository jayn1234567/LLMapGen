...
# Fresh-OBS, full end-to-end evaluation for the ordinary Raw-Lane local256
# 550k checkpoint.  The input is the already composited lane_patch_tif raster;
# do not draw Raw-Lane a second time over the image.
...
CHECKPOINT_CACHE_ROOT=${CHECKPOINT_CACHE_ROOT:-/cache/jn/checkpoints/rawlane_local256_550k_checkpoint34376}

RUN_ID=${RUN_ID:-rawlane_local256_550k_checkpoint34376_gt_empty_fresh_obs_e2e_$(date +%Y%m%d_%H%M%S)}
...
echo "============================================================"
echo "RAWLANE LOCAL256-550K CHECKPOINT-34376 FRESH OBS ORIGINAL E2E"
echo "Run id:          ${RUN_ID}"
...
echo "[rawlane550k-fresh-e2e] stage 1/3: force-download and extract clean E2E data"
rm -f "${FRESH_ARCHIVE}"
...
echo "[rawlane550k-fresh-e2e] stage 2/3: rebuild Raw-Lane local256 inputs and run inference"
...
echo "[rawlane550k-fresh-e2e] stage 3/3: suppress GT-empty patches and run original all/low/high evaluation"
...
echo "============================================================"
echo "RAWLANE LOCAL256-550K CHECKPOINT-34376 FRESH OBS E2E COMPLETE"
