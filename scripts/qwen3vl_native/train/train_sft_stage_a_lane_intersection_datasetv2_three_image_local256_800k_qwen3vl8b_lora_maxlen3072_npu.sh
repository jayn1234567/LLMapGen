...
# ============================================================
# DI/NPU LoRA max-length-3072 recipe for the released local256 Raw-Lane + Pose 800k dataset.
...
echo "[di-entry] reached local256-800k three-image native-Qwen3VL-8B LoRA maxlen3072 launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_three_image_local256_800k_native_qwen3vl8b_lora_maxlen3072_$(date -u +%Y%m%d_%H%M%S)
fi
...
MODEL_MAX_LENGTH=${MODEL_MAX_LENGTH:-3072}                                        # Lower text-sequence cap for the three-image per-device-batch-4 memory profile.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_three_image_local256_800k_native_qwen3vl8b_maxlen3072}  # Native local256 maxlen3072 group.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-native_qwen3vl8b_lora_three_image_local256_800k_maxlen3072}  # Experiment name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,phase_a,lane_intersection,datasetv2,three_image,local256,800k,native_qwen3vl8b,maxlen3072}  # Tags.
