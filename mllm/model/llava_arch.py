...
from mllm.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        self._ensure_view_type_embeddings()

    def _ensure_view_type_embeddings(self):
        if not bool(getattr(self.config, 'mm_use_view_type_embedding', False)):
            return

        count = int(getattr(self.config, 'mm_view_type_embedding_count', 2) or 2)
        if count < 1:
            raise ValueError("mm_view_type_embedding_count must be >= 1")

        hidden_size = int(self.config.hidden_size)
        current = getattr(self, 'mm_view_type_embeddings', None)
        if (
            isinstance(current, nn.Embedding)
            and current.num_embeddings >= count
            and current.embedding_dim == hidden_size
        ):
            self.config.mm_view_type_embedding_count = current.num_embeddings
            return

        new_embeddings = nn.Embedding(count, hidden_size)
        embed_std = float(getattr(self.config, 'mm_view_type_embedding_init_std', 0.0) or 0.0)
        if embed_std <= 0:
            embed_std = hidden_size ** -0.5
        nn.init.normal_(new_embeddings.weight, mean=0.0, std=embed_std)
        if isinstance(current, nn.Embedding):
            rows = min(current.num_embeddings, new_embeddings.num_embeddings)
            dims = min(current.embedding_dim, new_embeddings.embedding_dim)
            with torch.no_grad():
                new_embeddings.weight[:rows, :dims].copy_(current.weight[:rows, :dims])
        self.mm_view_type_embeddings = new_embeddings
        self.config.mm_use_view_type_embedding = True
        self.config.mm_view_type_embedding_count = count
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        for attr in (
            ...
            'mm_use_view_type_embedding',
            'mm_view_type_embedding_count',
            'mm_view_type_embedding_init_std',
        ):
...
class LlavaMetaForCausalLM(ABC):
    ...
    def _add_view_type_embeddings(self, image_features):
        if not bool(getattr(self.config, 'mm_use_view_type_embedding', False)):
            return image_features
        if image_features.ndim != 3:
            return image_features

        view_embeddings = getattr(self.get_model(), 'mm_view_type_embeddings', None)
        if view_embeddings is None:
            raise ValueError("mm_use_view_type_embedding=True but mm_view_type_embeddings is missing.")

        num_views = image_features.shape[0]
        if num_views > view_embeddings.num_embeddings:
            raise ValueError(
                f"Got {num_views} visual views but only "
                f"{view_embeddings.num_embeddings} view type embeddings are configured."
            )
        view_ids = torch.arange(num_views, device=image_features.device, dtype=torch.long)
        embeds = view_embeddings(view_ids).to(device=image_features.device, dtype=image_features.dtype)
        return image_features + embeds[:, None, :]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = [self._add_view_type_embeddings(x) for x in image_features]
            ...
            if mm_patch_merge_type == 'flat':
                ...
                if all_deepstack_features is not None:
                    all_deepstack_features = [
                        torch.stack([x.flatten(0, 1) for x in torch.split(layer_features, split_sizes, dim=0)], dim=0)
                        for layer_features in all_deepstack_features
                    ]
