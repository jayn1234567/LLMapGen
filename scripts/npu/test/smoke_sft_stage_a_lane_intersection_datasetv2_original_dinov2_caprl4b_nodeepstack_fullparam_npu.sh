...
# Single-node DI-like smoke for Dataset V2 local256 with original DINOv2 + CapRL.
# It reuses the formal DI launcher and only overrides runtime knobs.

SCRIPT_PATH=$(readlink -f "$0")
SCRIPT_DIR=$(dirname "${SCRIPT_PATH}")
REPO_ROOT=$(readlink -f "${SCRIPT_DIR}/../../..")
FORMAL_SCRIPT="${REPO_ROOT}/scripts/npu/train/train_sft_stage_a_lane_intersection_datasetv2_original_dinov2_caprl4b_nodeepstack_npu.sh"
cd "${REPO_ROOT}"
...
RUN_ID=${RUN_ID:-datasetv2_original_dinov2_caprl4b_di_like_smoke_$(date -u +%Y%m%d_%H%M%S)}
SMOKE_ROOT=${SMOKE_ROOT:-${OBS_CACHE}/outputs/datasetv2_original_dinov2_caprl4b_di_like_smoke}
...
DATASET_ARCHIVE_PATH=${DATASET_ARCHIVE_PATH:-${OBS_CACHE}/datasets/local256.tar}
DATASET_EXTRACT_ROOT=${DATASET_EXTRACT_ROOT:-${OBS_CACHE}/datasets/local256_extract}
...
echo "============================================================"
echo "Dataset V2 local256 original-DINOv2 + CapRL-4B DI-like smoke"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
