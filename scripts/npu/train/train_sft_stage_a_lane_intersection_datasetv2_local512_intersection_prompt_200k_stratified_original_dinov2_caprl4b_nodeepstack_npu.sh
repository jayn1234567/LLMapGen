...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 true-local512 stratified 200k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 true-local512 stratified-200k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_local512_intersection_prompt_200k_stratified_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/local_512_intersection_prompt/local512_intersection_prompt_550k.tar}  # Canonical true-local512 Dataset V2 package with GT intersections injected into the user prompt.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-local512_intersection_prompt}                # Preferred top-level directory stored in local512_intersection_prompt_550k.tar; resolved dynamically if different.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_local512_intersection_prompt_200k_stratified_original_dinov2_caprl4b}  # Group for the intersection-prompt exact-ratio subset run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_local512_intersection_prompt_200k_stratified_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,local512,intersection_prompt,200k,stratified,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
...
# Build the exact-ratio 200k training view without copying or changing any image.
FULL_TRAIN_PATH="${TRAIN_PATH}"
SUBSET_WORK_ROOT=${SUBSET_WORK_ROOT:-${DATASET_EXTRACT_ROOT}/.llmapgen_stratified_subsets}  # Node-local subset cache.
SUBSET_OUTPUT_PATH=${SUBSET_OUTPUT_PATH:-${SUBSET_WORK_ROOT}/train_${SUBSET_TARGET_SAMPLES}_seed${SUBSET_SEED}_rank${NODE_RANK}.jsonl}
SUBSET_SUMMARY_PATH=${SUBSET_SUMMARY_PATH:-${SUBSET_WORK_ROOT}/train_${SUBSET_TARGET_SAMPLES}_seed${SUBSET_SEED}_rank${NODE_RANK}.summary.json}
SUBSET_ARGS=(
  --input-jsonl "${FULL_TRAIN_PATH}"
  --output-jsonl "${SUBSET_OUTPUT_PATH}"
...
if [[ "${SUBSET_REUSE}" =~ ^(1|true|True|TRUE|yes|YES)$ ]]; then
  SUBSET_ARGS+=(--reuse-if-valid)
fi
...
if [ "${SUBSET_EXIT}" -ne 0 ]; then
  echo "ERROR: stratified subset construction failed with exit code ${SUBSET_EXIT}."
  exit "${SUBSET_EXIT}"
fi
if [ ! -s "${SUBSET_OUTPUT_PATH}" ] || [ ! -s "${SUBSET_SUMMARY_PATH}" ]; then
  echo "ERROR: stratified subset artifacts are missing after successful construction."
  exit 1
fi
TRAIN_PATH="${SUBSET_OUTPUT_PATH}"
cp "${SUBSET_SUMMARY_PATH}" "${OUTPUT_PATH}/train_subset_summary_rank${NODE_RANK}.json"
