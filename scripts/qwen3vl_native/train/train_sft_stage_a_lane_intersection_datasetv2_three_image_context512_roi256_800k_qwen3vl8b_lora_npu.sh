...
# ============================================================
# DI/NPU LoRA recipe for the released context512_roi256 Raw-Lane + Pose 800k dataset.
...
echo "[di-entry] reached context512_roi256-800k three-image native-Qwen3VL-8B LoRA launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_three_image_context512_roi256_800k_native_qwen3vl8b_lora_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/context512_roi256_rawpos/context512_roi256_rawlane_pose_800k.tar}  # Released context512_roi256 three-image 800k TAR.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-context512_roi256_rawlane_pose_800k}                  # Preferred root after extraction.
EXPECTED_DATASET_VARIANTS=${EXPECTED_DATASET_VARIANTS:-context512_roi256_rawlane_pose_800k,context512_roi256_rawlane_pose,rawlane_pose_three_image_context512_roi256_800k}  # Released identity plus historical aliases.
...
EXPECTED_CONTEXT_IMAGE_SIZE=${EXPECTED_CONTEXT_IMAGE_SIZE:-512}                  # All three native inputs are 512x512 context images.
EXPECTED_TARGET_SIZE=${EXPECTED_TARGET_SIZE:-256}                                # Labels and norm1000 coordinates are relative to the center ROI256.
EXPECTED_VIEW_MODE=${EXPECTED_VIEW_MODE:-context512_roi256,context_center_roi}    # Accepted record-level view-mode aliases.
...
ROI_PROMPT_TEXT=${ROI_PROMPT_TEXT:-"Coordinates are relative to the target ROI, not the full context image."}
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_three_image_context512_roi256_800k_native_qwen3vl8b}  # Native context512_roi256 group.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-native_qwen3vl8b_lora_three_image_context512_roi256_800k}  # Experiment name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,phase_a,lane_intersection,datasetv2,three_image,context512_roi256,800k,native_qwen3vl8b}  # Tags.
...
python - "${DATASET_PATH}" "${EXPECTED_DATASET_VARIANTS}" "${EXPECTED_SOURCE_TRAIN_SAMPLES}" "${TRAIN_SAMPLE_LIMIT}" "${EXPECTED_CONTEXT_IMAGE_SIZE}" "${EXPECTED_TARGET_SIZE}" "${EXPECTED_VIEW_MODE}" <<'PY'
import json
import sys
from pathlib import Path
...
expected_context_size = int(sys.argv[5])
expected_target_size = int(sys.argv[6])
expected_view_modes = {item.strip() for item in sys.argv[7].split(",") if item.strip()}
margin = (expected_context_size - expected_target_size) // 2
expected_roi = [margin, margin, margin + expected_target_size, margin + expected_target_size]
...
for line_number, line in enumerate(train_path.open("r", encoding="utf-8"), 1):
    ...
    context_size = int(meta.get("context_image_size", 0) or 0)
    target_size = int(meta.get("target_size", meta.get("patch_width", 0)) or 0)
    patch_width = int(meta.get("patch_width", target_size) or 0)
    patch_height = int(meta.get("patch_height", target_size) or 0)
    if context_size != expected_context_size:
        raise SystemExit(
            f"Sample {sample_id!r} context_image_size={context_size}, expected {expected_context_size}"
        )
    if target_size != expected_target_size or patch_width != expected_target_size or patch_height != expected_target_size:
        raise SystemExit(
            f"Sample {sample_id!r} target geometry is target={target_size}, "
            f"patch={patch_width}x{patch_height}; expected {expected_target_size}x{expected_target_size}"
        )
    if meta.get("target_roi_in_image") != expected_roi:
        raise SystemExit(
            f"Sample {sample_id!r} target_roi_in_image={meta.get('target_roi_in_image')!r}, "
            f"expected {expected_roi!r}"
        )
    if str(meta.get("view_mode") or "") not in expected_view_modes:
        raise SystemExit(
            f"Sample {sample_id!r} view_mode={meta.get('view_mode')!r}, "
            f"expected one of {sorted(expected_view_modes)!r}"
        )
    records += 1
...
print(
    f"[three-image-preflight] variant={actual_variant} source={records} selected={selected} "
    f"context={expected_context_size} target={expected_target_size} roi={expected_roi}"
)
PY
...
INSPECT_ARGS=(
  ...
  --required-prompt-text "${ROI_PROMPT_TEXT}"
...
echo "Images:       3 x 512 context images; center ROI256 supervision; native Qwen3-VL dynamic processor"
...
torchrun \
  ...
  --expected_context_image_size "${EXPECTED_CONTEXT_IMAGE_SIZE}" \
  --expected_target_size "${EXPECTED_TARGET_SIZE}" \
  --expected_view_mode "${EXPECTED_VIEW_MODE}" \
