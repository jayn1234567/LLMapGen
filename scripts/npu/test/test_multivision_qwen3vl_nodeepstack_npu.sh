...
# Formal NPU inference launcher for multi-vision no-DeepStack recipes.
...
CHECKPOINT_DIR=${CHECKPOINT_DIR:-}                                                # Single local checkpoint root for older inference launchers.
...
if [ -n "${CHECKPOINT_OBS_LIST}" ]; then
  while IFS= read -r obs_item; do
    ...
    resolved=$(python scripts/tools/resolve_best_checkpoint.py --output-dir "${local_dir}" --best-name infer_best --best-name eval_best --best-name best --best-name best_reward --allow-direct)
  ...
  while IFS= read -r local_item; do
    resolved=$(python scripts/tools/resolve_best_checkpoint.py --output-dir "${local_item}" --best-name infer_best --best-name eval_best --best-name best --best-name best_reward --allow-direct)
    CHECKPOINT_ITEMS+=("${resolved}")
    CHECKPOINT_LABELS+=("$(safe_label "${local_item}")")
  done < <(read_list "${CHECKPOINT_DIRS}")
elif [ -n "${CHECKPOINT_DIR}" ]; then
  resolved=$(python scripts/tools/resolve_best_checkpoint.py --output-dir "${CHECKPOINT_DIR}" --best-name infer_best --best-name eval_best --best-name best --best-name best_reward --allow-direct)
  CHECKPOINT_ITEMS+=("${resolved}")
  CHECKPOINT_LABELS+=("$(safe_label "${CHECKPOINT_DIR}")")
else
  echo "ERROR: set CHECKPOINT_OBS_LIST, CHECKPOINT_DIRS, or CHECKPOINT_DIR for inference."
  exit 1
fi
...
run_one_checkpoint() {
  ...
  if [ "${DATASET_PHASE}" = "phase_b" ]; then
    ...
    if [ "${MAP_TASK}" = "lane_intersection" ]; then INCLUDE_INTERSECTION_ARGS=(--include-intersections); fi
    torchrun \
  ...
  if [ "${NODE_RANK}" -ne 0 ]; then return 0; fi
