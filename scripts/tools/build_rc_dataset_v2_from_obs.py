...
def parse_args(argv=None):
    ...
    parser.add_argument("--work-root", default="/cache/rc_dataset_v2")
    ...
    parser.add_argument("--output-obs-root", default="", help="Destination OBS directory. Empty disables upload.")
    ...
    parser.add_argument("--difficulty-ratios", default="empty=0.05,easy=0.30,medium=0.30,hard=0.25,very_hard=0.10")
...
def download_sources(sources, raw_root, resume):
    mox = import_moxing()
    local_roots = []
    for index, source in enumerate(sources):
        ...
        mox.file.copy_parallel(source, str(local_root))
        marker.write_text(
...
def upload_outputs(output_root, output_obs_root, views, upload_mode, resume):
    mox = import_moxing()
    variants = []
...
def main(argv=None):
    ...
    if args.skip_download:
        ...
        local_roots = download_sources(sources, raw_root, args.resume)
