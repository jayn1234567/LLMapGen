...
# ============================================================
# NPU SFT training
# Fixed recipe: Jiangjihua v9 original 33w | latest private DINOv2 last2 + CapRL-Qwen3VL-4B | discrete coordinates
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Jiangjihua original-33w latest-private-DINOv2 last2 CapRL discrete-coordinate launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=jjh33w_latest_private_dinov2_last2_caprl4b_coordtokens_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-256}                  # Raw image width and height in Jiangjihua's original dataset.
DATASET_COORD_MAX=${DATASET_COORD_MAX:-1000}                                     # Expected norm1000 target-coordinate upper bound.
DATASET_ALLOWED_TARGET_LANE_TYPES=${DATASET_ALLOWED_TARGET_LANE_TYPES:-}          # The original targets do not require a centerline type field.
...
COORDINATE_TOKEN_MODE=angle                                                       # Fixed recipe: encode every points coordinate as one ordinary token.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_jjh33w_private_dinov2_caprl4b_coordtokens}  # Discrete-coordinate experiment group.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_jjh33w_latest_private_dinov2_last2_caprl4b_coordtokens_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,jjh33w,dinov2,last2,caprl4b,coordtokens,nodeepstack}  # Comma-separated SwanLab tags.
...
# This is Jiangjihua's original prepared norm1000 dataset. DI only extracts and verifies it.
