...
# ============================================================
# NPU SFT training
# Fixed recipe: phase_a_oracle_intersection | lane-only centerline conditioned on GT intersections | DINOv3 main visual stream, no DeepStack | Qwen3 text LLM
...
SOURCE_DATASET_PHASE=${SOURCE_DATASET_PHASE:-phase_a}                            # Source phase that still contains lane+intersection assistant targets.
DATASET_PHASE=phase_a_oracle_intersection                                         # Converted phase: GT intersections are moved into the user prompt.
MAP_TASK=lane                                                                     # Task type: lane or lane_intersection. Oracle-intersection target is lane-only.
...
ORACLE_SPLITS=${ORACLE_SPLITS:-train,eval,test}                                   # Splits converted from SOURCE_DATASET_PHASE into DATASET_PHASE.
ORACLE_COORD_MAX=${ORACLE_COORD_MAX:-512}                                         # Fallback coordinate upper bound when records do not carry coord metadata.
...
echo "[oracle-data] source phase: ${DATASET_PATH}/${SOURCE_DATASET_PHASE}"
echo "[oracle-data] output phase: ${DATASET_PATH}/${DATASET_PHASE}"
python scripts/tools/build_oracle_intersection_centerline_dataset.py \
  --input-root "${DATASET_PATH}" \
  --output-root "${DATASET_PATH}" \
  --source-phase "${SOURCE_DATASET_PHASE}" \
  --output-phase "${DATASET_PHASE}" \
  --splits "${ORACLE_SPLITS}" \
  --coord-max "${ORACLE_COORD_MAX}" \
  --overwrite
...
echo "Source phase: ${SOURCE_DATASET_PHASE}"
echo "Oracle data:  intersections are prompt-only; assistant target is centerline-only"
