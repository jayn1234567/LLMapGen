...
def run_patch_inference(record, prompt_text, image_folder, tokenizer, model, image_processor,
                        ...
                        context_image_key="context_image", require_context_image=False):
    image_path = resolve_image_path(record["image"], image_folder).resolve()
    image = Image.open(image_path).convert("RGB")
    input_images = []
    context_image = record.get(context_image_key)
    if context_image:
        context_path = resolve_image_path(context_image, image_folder).resolve()
        input_images.append(Image.open(context_path).convert("RGB"))
    elif require_context_image:
    ...
    for local_idx, record in enumerate(records):
        ...
        if args.dry_run_prompts:
            ...
            image_path, prompt, raw_prediction, prediction, input_token_len, output_token_len, decoded_token_len, decoded_mode = run_patch_inference(
                ...
                args.max_new_tokens,
                args.temperature,
                context_image_key=args.context_image_key,
                require_context_image=args.require_context_image,
            )
