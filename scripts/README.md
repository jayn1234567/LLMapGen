...
not run the raw converter unless `DATASET_KIND=raw` is set explicitly. It also
skips trainroot validation by default; set `VALIDATE_TRAINROOT=true` only when
