...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane local256 full 550k | original DINOv2 + CapRL-Qwen3VL-4B LLM LoRA | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 Raw-Lane local256 full-550k original-DINOv2 CapRL LLM-LoRA SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_rawlane_local256_550k_original_dinov2_caprl4b_lora_llm_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/rawlane_local256_550k/rawlane_local256_550k.tar}  # Final preselected Raw-Lane local256 550k package.
...
DATASET_DIR_NAME=${DATASET_DIR_NAME:-rawlane_local256_550k}                     # Preferred archive root; dynamic resolution handles a differently named single dataset root.
...
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-550000}                         # The OBS package is already difficulty-stratified; DI must consume it unchanged.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_rawlane_local256_550k_original_dinov2_caprl4b_lora_llm}  # Group for this LoRA run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_rawlane_local256_550k_original_dinov2_caprl4b_lora_llm_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,llm_lora,phase_a,lane_intersection,datasetv2,rawlane,local256,550k,dinov2,original,caprl4b,nodeepstack,lr2e-4}  # Comma-separated SwanLab tags.
...
# The OBS archive is already the final deterministic 550k selection. Validate it
