...
# Formal DI recipe for full-parameter private-road DINOv3-L/16 segmentation.
...
echo "[di-entry] reached formal DINOv3 private segmentation training script"
...
OBS_CACHE=${OBS_CACHE:-/cache/llmapgen_dinov3_seg}
MODEL_OBS_PATH=${MODEL_OBS_PATH:-obs://yw-ads-training-gy1/data/external/personal/h58801830/whu/jjh/checkpoints/facebook_dinov3-vitl16-pretrain-lvd1689m}
MODEL_LOCAL_PATH=${MODEL_LOCAL_PATH:-${OBS_CACHE}/models/facebook_dinov3-vitl16-pretrain-lvd1689m}
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=dinov3_private_seg_fullft_$(date -u +%Y%m%d_%H%M%S)
fi
RUN_ID=${RUN_ID:-${DEFAULT_RUN_ID}}
LOCAL_OUTPUT_ROOT=${LOCAL_OUTPUT_ROOT:-/cache/local_model_save_path}
OUTPUT_DIR=${OUTPUT_DIR:-${LOCAL_OUTPUT_ROOT}/${RUN_ID}}
if [ -z "${OUTPUT_URL:-}" ]; then
  OUTPUT_URL=obs://yw-ads-training-gy1/data/external/personal/h58801830/whu/jn/model/dinov3_private_seg_fullft
fi
...
if [ "${EXPECTED_WORLD_SIZE}" -gt 0 ] && [ "${TOTAL_DEVICES}" -ne "${EXPECTED_WORLD_SIZE}" ]; then
  echo "ERROR: this formal recipe expects ${EXPECTED_WORLD_SIZE} devices, got ${TOTAL_DEVICES}."
  echo "Set EXPECTED_WORLD_SIZE=0 only for an intentional smoke test."
  exit 1
fi

echo "============================================================"
echo "Formal DINOv3 private segmentation training"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
...
"${PYTHON}" -m torch.distributed.run \
  "${TORCHRUN_ARGS[@]}" \
  -m mllm.vision_pretrain.train_dinov3_segmentation \
...
payload = {
    ...
    "expected_visual_tokens": 1024,
    "expected_hidden_size": 1024,
    "normalization_mode": "minus_half",
}
(output_dir / "best" / "jiangjihua_vision_tower.json").write_text(
    json.dumps(payload, indent=2, ensure_ascii=True), encoding="utf-8"
)
print(f"[jiangjihua-compat] {json.dumps(payload, ensure_ascii=True)}", flush=True)
PY

  "${PYTHON}" scripts/tools/verify_dinov3_vision_tower.py \
    --vision-tower "${VISION_TOWER_DIR}" \
    --device npu \
    --input-size 512 \
    --select-layer 24 \
    --expected-tokens 1024 \
    --expected-hidden-size 1024 \
    --expected-num-layers 24 \
    --expected-register-tokens 4 \
...
required = (
    ...
    f"{target}/best/jiangjihua_vision_tower.json",
...
PY
  ...
  echo "============================================================"
  echo "Formal DI DINOv3 segmentation training PASSED"
