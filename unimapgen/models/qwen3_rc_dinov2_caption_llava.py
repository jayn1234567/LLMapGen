...
class Qwen3RCDinoCaptionModel(nn.Module):
    def __init__(
        self,
        *,
        ...
        train_full_token_embeddings: bool = False,
    ) -> None:
        super().__init__()
        if AutoModelForCausalLM is None:
            raise RuntimeError(
                "Qwen3RCDinoCaptionModel failed to import AutoModelForCausalLM. "
        ...
        if bool(freeze_language_model):
            ...
            # Stage 2 只放开视觉边界 token，让 Qwen 感知视觉块的起止，不改动主体语言能力。
            self._enable_selective_trainable_tokens(self.trainable_visual_special_token_ids)
            self._set_selective_token_trainable(True)
            print(
                (
                    "[qwen3-rc-caption] selective token tuning enabled for visual boundary tokens: "
                    f"tokens={TRAINABLE_VISUAL_SPECIAL_TOKENS}"
                ),
                flush=True,
            )
        elif not bool(train_full_token_embeddings):
            for module in filter(None, [self.language_model.get_input_embeddings(), self.language_model.get_output_embeddings()]):
        ...
        # Stage 2 直接复用 Stage 1 学到的 bridge，不再叠加新的视觉注入模块。
    ...
    def inject_visual_embeddings(
        ...
        *,
        ...
        # <vis_patch> 只是占位符；真正进入 Qwen 的是 bridge 生成出来的视觉 embedding。
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # 训练目标是在 frozen Qwen 里让视觉桥支撑结构化 caption 生成。
...
def save_qwen3_rc_dinov2_caption_modules(
    model: nn.Module,
    output_dir: str | Path,
    ...
    torch.save(
        ...
        str(output_path / "rc_dinov2_caption_modules.pt"),
    )
