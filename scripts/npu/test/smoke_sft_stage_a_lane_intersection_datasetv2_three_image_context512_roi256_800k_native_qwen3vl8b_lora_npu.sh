...
# context512_roi256 Raw-Lane + Pose three-image 800k dataset.
# It reuses the formal DI launcher and only overrides runtime knobs.

SCRIPT_PATH=$(readlink -f "$0")
SCRIPT_DIR=$(dirname "${SCRIPT_PATH}")
REPO_ROOT=$(readlink -f "${SCRIPT_DIR}/../../..")
FORMAL_SCRIPT="${REPO_ROOT}/scripts/qwen3vl_native/train/train_sft_stage_a_lane_intersection_datasetv2_three_image_context512_roi256_800k_qwen3vl8b_lora_npu.sh"
cd "${REPO_ROOT}"
...
RUN_ID=${RUN_ID:-datasetv2_three_image_context512_roi256_800k_native_qwen3vl8b_di_like_smoke_$(date -u +%Y%m%d_%H%M%S)}
SMOKE_ROOT=${SMOKE_ROOT:-${OBS_CACHE}/outputs/datasetv2_three_image_context512_roi256_800k_native_qwen3vl8b_di_like_smoke}
...
echo "============================================================"
echo "Dataset V2 three-image context512_roi256-800k native Qwen3-VL-8B LoRA smoke"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
...
echo "Native vision:    Qwen3-VL dynamic 3 x 512 context input; center ROI256 supervision"
...
if ! grep -Fq "context=512 target=256 roi=[128, 128, 384, 384]" "${TRAIN_LOG}"; then
  echo "ERROR: the context512/ROI256 dataset geometry preflight was not confirmed." >&2
  exit 1
fi
if ! grep -Fq "expected_geometry=context=512 target=256" "${TRAIN_LOG}"; then
  echo "ERROR: the runtime DataLoader geometry gate was not enabled." >&2
  exit 1
fi
