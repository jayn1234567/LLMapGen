...
class ModelArguments:
    ...
    vision_tower_checkpoint_path: Optional[str] = field(
        default=None,
        metadata={"help": "Optional external vision-tower weights loaded after the base DINO/vision tower is built."},
    )
    ...
    mm_use_view_type_embedding: bool = field(
        default=False,
        metadata={"help": "Add learned view type embeddings to multi-view image tokens before flattening."},
    )
    mm_view_type_embedding_count: int = field(
        default=2,
        metadata={"help": "Number of learned view type embeddings. For global-local training use 2: context, local."},
    )
    mm_view_type_embedding_init_std: float = field(
        default=0.0,
        metadata={"help": "Initialization std for view type embeddings. <=0 uses hidden_size^-0.5."},
    )
...
class DataArguments:
    ...
    context_image_key: str = field(
        default="context_image",
        metadata={"help": "JSON field for the context/global view image. When present, dataset returns [context, local]."},
    )
    require_context_image: bool = field(
        default=False,
        metadata={"help": "Require context_image_key for every multimodal sample."},
    )
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        # Only save Adapter
        keys_to_match = ['mm_projector', 'mm_view_type_embeddings']
...
class LazySupervisedDataset(Dataset):
    ...
    @staticmethod
    def _resolve_image_path(image_folder: str, image_file: str) -> str:
        if os.path.isabs(image_file):
            return image_file
        return os.path.join(image_folder, image_file)

    def _load_and_preprocess_image(self, image_file: str, image_folder: str):
        processor = self.data_args.image_processor
        image = Image.open(self._resolve_image_path(image_folder, image_file)).convert('RGB')
    ...
    def get_sample(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            context_image_key = getattr(self.data_args, "context_image_key", "context_image")
            context_image_file = self.list_data_dict[i].get(context_image_key)
            if getattr(self.data_args, "require_context_image", False) and not context_image_file:
                raise ValueError(f"Missing required context image field: {context_image_key}")
            img_path_idx = self.list_data_dict[i]['img_path_idx']
            image_folder = self.data_args.image_folder[img_path_idx]
            image, image_size = self._load_and_preprocess_image(image_file, image_folder)
            if context_image_file:
                context_image, _ = self._load_and_preprocess_image(context_image_file, image_folder)
                if context_image.shape != image.shape:
                    raise ValueError(
                        f"Context and local images must preprocess to the same shape, got "
                        f"{tuple(context_image.shape)} and {tuple(image.shape)}"
                    )
                image = torch.stack([context_image, image], dim=0)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.tune_mm_mlp_adapter:
            ...
            view_embeddings = getattr(model.get_model(), "mm_view_type_embeddings", None)
            if view_embeddings is not None:
                for p in view_embeddings.parameters():
                    p.requires_grad = True
        ...
        if training_args.bits in [4, 8]:
            ...
            view_embeddings = getattr(model.get_model(), "mm_view_type_embeddings", None)
            if view_embeddings is not None:
                view_embeddings.to(dtype=compute_dtype, device=training_args.device)
