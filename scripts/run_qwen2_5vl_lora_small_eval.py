...
def generate_with_llamafactory_engine(
    ...
    return (responses[0].response_text if responses else ""), None
...
def main() -> None:
    ...
    for idx, sample in enumerate(selected, start=1):
        ...
        state_lines = sanitize_lines(extract_state_lines(sample))
        ...
        if (
            idx == 1
            or idx == total_selected
            or (int(args.progress_every) > 0 and idx % int(args.progress_every) == 0)
        ):
            elapsed_sec = max(0.0, time.time() - started_at)
            avg_sec = elapsed_sec / max(1, idx)
            remaining_sec = avg_sec * max(0, total_selected - idx)
            print(
                json.dumps(
                    {
                        "event": "progress",
    ...
    paper_categories = [x.strip() for x in str(args.paper_categories).split(",") if x.strip()]
