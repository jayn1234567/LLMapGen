...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 context512_roi256 stratified 200k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 context512_roi256 stratified-200k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_context512_roi256_200k_stratified_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/context512_roi256/context512_roi256_550k.tar}  # Canonical context512_roi256 Dataset V2 550k package.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-context512_roi256}                                    # Top-level directory stored in context512_roi256_550k.tar.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-512}                  # Raw image width and height in the context512_roi256 dataset.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_context512_roi256_200k_stratified_original_dinov2_caprl4b}  # Group for this exact-ratio subset run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_context512_roi256_200k_stratified_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,context512_roi256,200k,stratified,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
