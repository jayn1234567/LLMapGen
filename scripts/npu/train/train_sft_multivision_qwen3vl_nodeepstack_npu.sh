...
# Formal NPU SFT launcher for multi-vision no-DeepStack recipes.
# Required recipe variables are usually set by the thin wrapper scripts:
#   DATASET_PHASE=phase_a|phase_b
#   MAP_TASK=lane|lane_intersection
#   VISION_BACKBONE=multi_moe|dinov2_siglip_concat|dinov3_siglip_concat
...
DATASET_PHASE=${DATASET_PHASE:-phase_a}                                           # Dataset stage: phase_a for patch inference, phase_b for state update.
MAP_TASK=${MAP_TASK:-lane}                                                        # Task type: lane or lane_intersection.
VISION_BACKBONE=${VISION_BACKBONE:-multi_moe}                                     # Visual backbone selector used by the generic multi-vision launcher.
...
DINO_V2_TOWER_NAME=${DINO_V2_TOWER_NAME:-facebook_dinov2-large}                   # DINOv2 checkpoint directory name under MODEL_OBS_PATH.
...
SIGLIP_TOWER_NAME=${SIGLIP_TOWER_NAME:-google_siglip-large-patch16-384}           # SigLIP checkpoint directory name under MODEL_OBS_PATH.
DINO_V2_TOWER=${DINO_V2_TOWER:-${OBS_CACHE}/checkpoints/${DINO_V2_TOWER_NAME}}    # Local DINOv2 tower path. Download is skipped when config.json exists.
DINO_V3_TOWER=${DINO_V3_TOWER:-${OBS_CACHE}/checkpoints/${DINO_V3_TOWER_NAME}}    # Local DINOv3 tower path. Download is skipped when config.json exists.
SIGLIP_TOWER=${SIGLIP_TOWER:-${OBS_CACHE}/checkpoints/${SIGLIP_TOWER_NAME}}       # Local SigLIP tower path. Download is skipped when config.json exists.
...
case "${VISION_BACKBONE}" in
  multi_moe|multi_vision_moe|dual_dino_moe)
    MULTI_VISION_TOWERS=${MULTI_VISION_TOWERS:-${DINO_V2_TOWER},${DINO_V3_TOWER}} # Comma-separated vision tower paths for multi-vision recipes.
    MULTI_VISION_TOWER_TYPES=${MULTI_VISION_TOWER_TYPES:-dinov2,dinov3}           # Tower type list aligned with MULTI_VISION_TOWERS.
    MULTI_VISION_INPUT_IMAGE_SIZES=${MULTI_VISION_INPUT_IMAGE_SIZES:-512,512}     # Input image size per tower, aligned with MULTI_VISION_TOWERS.
    MULTI_VISION_PRIMARY_INDEX=${MULTI_VISION_PRIMARY_INDEX:-1}                   # Tower index used as the primary processor/grid reference.
    MULTI_VISION_HIDDEN_SIZE=${MULTI_VISION_HIDDEN_SIZE:-1024}                    # Shared hidden size after each vision tower adapter.
    MULTI_VISION_TARGET_GRID=${MULTI_VISION_TARGET_GRID:-32}                      # Target token grid side length after visual feature alignment.
    MULTI_VISION_FUSION=${MULTI_VISION_FUSION:-softmax_router}                    # Fusion method: softmax_router for MoE or concat_projector for concat.
    MM_VISION_TOWER_TYPE=multi_moe                                                # Model-side vision tower type: dinov2, dinov3, multi_moe, or multi_concat.
    INPUT_IMAGE_SIZE=${INPUT_IMAGE_SIZE:-512}                                     # Image size fed to the vision encoder; DINOv3 recipes usually use 512.
    DOWNLOAD_TOWER_NAMES=("${DINO_V2_TOWER_NAME}" "${DINO_V3_TOWER_NAME}")        # Vision tower names to download for this multi-vision recipe.
    REQUIRED_VISION_TOWERS=("${DINO_V2_TOWER}" "${DINO_V3_TOWER}")                # Local vision tower paths that must exist before launch.
    ;;
  dinov2_siglip_concat|dinov2_siglip|dinosiglip_v2)
    MULTI_VISION_TOWERS=${MULTI_VISION_TOWERS:-${DINO_V2_TOWER},${SIGLIP_TOWER}}  # Comma-separated vision tower paths for multi-vision recipes.
    MULTI_VISION_TOWER_TYPES=${MULTI_VISION_TOWER_TYPES:-dinov2,siglip}           # Tower type list aligned with MULTI_VISION_TOWERS.
    MULTI_VISION_INPUT_IMAGE_SIZES=${MULTI_VISION_INPUT_IMAGE_SIZES:-512,384}     # Input image size per tower, aligned with MULTI_VISION_TOWERS.
    MULTI_VISION_PRIMARY_INDEX=${MULTI_VISION_PRIMARY_INDEX:-0}                   # Tower index used as the primary processor/grid reference.
    MULTI_VISION_HIDDEN_SIZE=${MULTI_VISION_HIDDEN_SIZE:-1024}                    # Shared hidden size after each vision tower adapter.
    MULTI_VISION_TARGET_GRID=${MULTI_VISION_TARGET_GRID:-32}                      # Target token grid side length after visual feature alignment.
    MULTI_VISION_FUSION=${MULTI_VISION_FUSION:-concat_projector}                  # Fusion method: softmax_router for MoE or concat_projector for concat.
    MM_VISION_TOWER_TYPE=multi_concat                                             # Model-side vision tower type: dinov2, dinov3, multi_moe, or multi_concat.
    INPUT_IMAGE_SIZE=${INPUT_IMAGE_SIZE:-512}                                     # Image size fed to the vision encoder; DINOv3 recipes usually use 512.
    DOWNLOAD_TOWER_NAMES=("${DINO_V2_TOWER_NAME}" "${SIGLIP_TOWER_NAME}")         # Vision tower names to download for this multi-vision recipe.
    REQUIRED_VISION_TOWERS=("${DINO_V2_TOWER}" "${SIGLIP_TOWER}")                 # Local vision tower paths that must exist before launch.
    ;;
  dinov3_siglip_concat|dinov3_siglip|dinosiglip_v3)
    MULTI_VISION_TOWERS=${MULTI_VISION_TOWERS:-${DINO_V3_TOWER},${SIGLIP_TOWER}}  # Comma-separated vision tower paths for multi-vision recipes.
    MULTI_VISION_TOWER_TYPES=${MULTI_VISION_TOWER_TYPES:-dinov3,siglip}           # Tower type list aligned with MULTI_VISION_TOWERS.
    MULTI_VISION_INPUT_IMAGE_SIZES=${MULTI_VISION_INPUT_IMAGE_SIZES:-512,384}     # Input image size per tower, aligned with MULTI_VISION_TOWERS.
    MULTI_VISION_PRIMARY_INDEX=${MULTI_VISION_PRIMARY_INDEX:-0}                   # Tower index used as the primary processor/grid reference.
    MULTI_VISION_HIDDEN_SIZE=${MULTI_VISION_HIDDEN_SIZE:-1024}                    # Shared hidden size after each vision tower adapter.
    MULTI_VISION_TARGET_GRID=${MULTI_VISION_TARGET_GRID:-32}                      # Target token grid side length after visual feature alignment.
    ...
    MM_VISION_TOWER_TYPE=multi_concat                                             # Model-side vision tower type: dinov2, dinov3, multi_moe, or multi_concat.
    INPUT_IMAGE_SIZE=${INPUT_IMAGE_SIZE:-512}                                     # Image size fed to the vision encoder; DINOv3 recipes usually use 512.
    DOWNLOAD_TOWER_NAMES=("${DINO_V3_TOWER_NAME}" "${SIGLIP_TOWER_NAME}")         # Vision tower names to download for this multi-vision recipe.
    REQUIRED_VISION_TOWERS=("${DINO_V3_TOWER}" "${SIGLIP_TOWER}")                 # Local vision tower paths that must exist before launch.
    ;;
  *) echo "ERROR: unsupported VISION_BACKBONE=${VISION_BACKBONE}"; exit 1 ;;
esac
VISION_TOWER=${VISION_TOWER:-${MULTI_VISION_TOWERS}}                              # Vision tower path passed to the model loader. Multi-vision uses a comma list.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_${DATASET_PHASE}_${MAP_TASK}_${VISION_BACKBONE}_nodeepstack}  # SwanLab group name for related runs.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_${DATASET_PHASE}_${MAP_TASK}_${VISION_BACKBONE}_qwen3vl8b_nodeepstack}  # SwanLab experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,${DATASET_PHASE},${MAP_TASK},${VISION_BACKBONE},qwen3vl8b,nodeepstack,unimapgen_v9}  # Comma-separated SwanLab tags for filtering runs.
...
export RDZV_ID=${RDZV_ID:-sft_${DATASET_PHASE}_${MAP_TASK}_${VISION_BACKBONE}_${RUN_ID}}  # Unique rendezvous id for this distributed run.
...
if [ "${DATASET_PHASE}" = "phase_b" ]; then
  ...
  INIT_MODEL_PATH=$(python scripts/tools/resolve_best_checkpoint.py --output-dir "${CHECKPOINT_INPUT_PATH}" --best-name infer_best --best-name eval_best --best-name best --best-name best_reward --allow-direct)  # Initial model path passed to train_qwen. Stage B uses the Stage-A checkpoint.
else
...
VISION_ARGS=(                                                                     # Model vision arguments shared by train or inference launch commands.
