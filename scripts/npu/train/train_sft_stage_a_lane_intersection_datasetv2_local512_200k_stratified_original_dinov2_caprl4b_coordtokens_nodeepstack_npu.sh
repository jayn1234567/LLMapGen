...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_local512_200k_stratified_original_dinov2_caprl4b_coordtokens_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/local512/local512_550k.tar}  # Canonical true-local512 Dataset V2 550k package.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-local512}                                    # Top-level directory stored in local512_550k.tar.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-512}                  # Raw image width and height in the true-local512 dataset.
...
COORDINATE_TOKEN_MODE=angle                                                       # Fixed recipe: encode every map coordinate as one <0>...<1000> token.
COORDINATE_TOKEN_MAX=1000                                                        # Fixed norm1000 vocabulary: <0> through <1000>.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_local512_200k_stratified_original_dinov2_caprl4b_coordtokens}  # Group for this discrete-coordinate exact-ratio subset run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_local512_200k_stratified_original_dinov2_caprl4b_coordtokens_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,local512,200k,stratified,dinov2,original,caprl4b,coordtokens,nodeepstack}  # Comma-separated SwanLab tags.
