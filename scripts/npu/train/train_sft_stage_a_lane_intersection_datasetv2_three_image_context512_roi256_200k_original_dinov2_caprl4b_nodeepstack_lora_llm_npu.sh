...
TRAIN_SAMPLE_LIMIT=${TRAIN_SAMPLE_LIMIT:-200000}                                  # Deterministic subset shared with the native Qwen3-VL comparison.
SAMPLE_SEED=${SAMPLE_SEED:-42}                                                    # Both A/B recipes use the same Python random sampling algorithm and seed.
...
NUM_EPOCHS=${NUM_EPOCHS:-8}                                                       # Shared A/B epoch budget.
...
# Validate the source package. The trainer then applies the same deterministic
# random-index selection as the native Qwen3-VL dataset adapter.
TRAIN_DATASET_SUMMARY=${TRAIN_DATASET_SUMMARY:-${OUTPUT_PATH}/train_dataset_summary_rank${NODE_RANK}.json}
python - "${TRAIN_PATH}" "${EXPECTED_TRAIN_SAMPLES}" "${TRAIN_SAMPLE_LIMIT}" "${SAMPLE_SEED}" "${TRAIN_DATASET_SUMMARY}" <<'PY'
import json
import sys
from pathlib import Path
...
if selected <= 0 or selected > records:
    raise SystemExit(f"Invalid matched subset size {selected}; source contains {records} records")
payload = {
    ...
    "sample_seed": seed,
    "selection": "random.Random(seed).sample sorted by source index",
}
summary_path.parent.mkdir(parents=True, exist_ok=True)
summary_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
print(f"[three-image-preflight] source passed: records={records}; matched subset={selected}; seed={seed}")
PY
...
echo "Train asset:  source_records=${EXPECTED_TRAIN_SAMPLES}, selected_records=${TRAIN_SAMPLE_LIMIT}, seed=${SAMPLE_SEED}"
echo "Eval:         ${EVAL_PATH}"
