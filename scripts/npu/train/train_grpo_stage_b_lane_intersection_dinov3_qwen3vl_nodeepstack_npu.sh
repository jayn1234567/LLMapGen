...
LOCAL_OUTPUT_DIR=${LOCAL_OUTPUT_DIR:-${OBS_CACHE}/grpo_phase_b_lane_intersection_dinov3_output_${RUN_ID}}  # Per-run local GRPO output directory.
...
REWARD_INTERSECTION_WEIGHT=${REWARD_INTERSECTION_WEIGHT:-0.10}                    # Extra reward weight for intersection fields. Use 0 for lane-only.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-grpo_phase_b_lane_intersection_dinov3_nodeepstack} # SwanLab group name for related runs.
...
SWANLAB_TAGS=${SWANLAB_TAGS:-grpo,phase_b,lane_intersection,dinov3,qwen3vl8b,nodeepstack,vllm-ascend}  # Comma-separated SwanLab tags for filtering runs.
