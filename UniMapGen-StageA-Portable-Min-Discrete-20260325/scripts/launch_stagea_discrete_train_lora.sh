...
OUTPUT_DIR=${OUTPUT_DIR:-$ROOT/outputs/stagea_discrete_tokens_lora}
...
if [[ ! -d "$MODEL_DIR" ]]; then
  echo "[portable-dtok-lora-train] missing model dir: $MODEL_DIR" >&2
  exit 1
fi
if [[ ! -f "$TRAIN_JSON" ]]; then
  echo "[portable-dtok-lora-train] missing train jsonl: $TRAIN_JSON" >&2
  exit 1
fi
if [[ ! -d "$DATASET_ROOT/images" ]]; then
  echo "[portable-dtok-lora-train] missing images dir: $DATASET_ROOT/images" >&2
  exit 1
fi
