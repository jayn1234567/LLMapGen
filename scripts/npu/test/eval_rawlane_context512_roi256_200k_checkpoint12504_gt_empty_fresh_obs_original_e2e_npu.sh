...
CHECKPOINT_OBS_PATH=${CHECKPOINT_OBS_PATH:-obs://yw-ads-model-training-gy1/model-dev/rc-nn/rc_base_model/2026/07/30/ea77c85a9d54442b825b86cd7f547a26/output/ma-job-2e7c82dd-3a05-440b-b686-db5c3bcc2512/checkpoint-12504/}
CHECKPOINT_CACHE_ROOT=${CHECKPOINT_CACHE_ROOT:-/cache/jn/checkpoints/rawlane_context512_roi256_200k_checkpoint12504}

RUN_ID=${RUN_ID:-rawlane_context512_roi256_200k_checkpoint12504_gt_empty_fresh_obs_e2e_$(date +%Y%m%d_%H%M%S)}
...
INFERENCE_DATASET_ROOT=${INFERENCE_DATASET_ROOT:-${FRESH_RUN_ROOT}/rawlane_context512_roi256_dataset}
...
echo "============================================================"
echo "RAWLANE CONTEXT512/ROI256-200K GT-EMPTY + FRESH OBS E2E"
echo "Run id:          ${RUN_ID}"
...
echo "Input/target:     RawLane context512 / center ROI256"
echo "Output:           ${OUTPUT_ROOT}"
echo "Evaluations:      all + low + high"
echo "============================================================"

echo "[rawlane-context-fresh-e2e] stage 1/3: force-download and extract clean E2E data"
rm -f "${FRESH_ARCHIVE}"
python - "${E2E_DATA_OBS_PATH}" "${FRESH_ARCHIVE}" <<'PY'
import sys
import moxing as mox

print(f"[rawlane-context-fresh-e2e] download {sys.argv[1]} -> {sys.argv[2]}", flush=True)
...
echo "[rawlane-context-fresh-e2e] stage 2/3: rebuild RawLane context512/ROI256 inputs and run fresh inference"
E2E_DATA_OBS_PATH="${E2E_DATA_OBS_PATH}" \
E2E_WORK_ROOT="${FRESH_RUN_ROOT}" \
E2E_ARCHIVE_PATH="${FRESH_ARCHIVE}" \
E2E_RAW_ROOT="${FRESH_E2E_ROOT}" \
E2E_VIEW_MODE=context512_roi256 \
E2E_TARGET_SIZE=256 \
E2E_CONTEXT_SIZE=512 \
E2E_PROMPT_PROFILE=rawlane_context512_roi256_200k_v1 \
...
echo "[rawlane-context-fresh-e2e] stage 3/3: count/suppress GT-empty patches and run original all/low/high E2E"
...
echo "============================================================"
echo "RAWLANE CONTEXT512/ROI256-200K FRESH E2E COMPLETE"
