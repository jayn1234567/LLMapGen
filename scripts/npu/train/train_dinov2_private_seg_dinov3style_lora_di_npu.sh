...
VISION_LEARNING_RATE=${VISION_LEARNING_RATE:-3e-3}
DECODER_LEARNING_RATE=${DECODER_LEARNING_RATE:-3e-3}
...
HIDDEN_STATE_INDICES=${HIDDEN_STATE_INDICES:-"21 22 23 24"}
PROJECTION_CHANNELS=${PROJECTION_CHANNELS:-128}
...
VISION_LORA_R=${VISION_LORA_R:-3}
VISION_LORA_ALPHA=${VISION_LORA_ALPHA:-3}
...
if [ "${GRADIENT_ACCUMULATION_STEPS}" -lt 1 ]; then
  echo "ERROR: GRADIENT_ACCUMULATION_STEPS must be >= 1."
  exit 1
fi
...
echo "Projection chans: ${PROJECTION_CHANNELS}"
...
echo "Numerics:         bf16=${BF16} gradient_checkpointing=${GRADIENT_CHECKPOINTING} max_grad_norm=${MAX_GRAD_NORM}"
