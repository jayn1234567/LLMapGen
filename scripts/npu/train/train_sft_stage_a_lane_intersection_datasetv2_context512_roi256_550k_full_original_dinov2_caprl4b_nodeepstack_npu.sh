...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 context512_roi256 full 550k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 context512_roi256 full-550k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_context512_roi256_550k_full_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-550000}                          # Guard against accidentally training on a 100k/200k package or stale subset.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_context512_roi256_550k_full_original_dinov2_caprl4b}  # Group for the full 550k run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_context512_roi256_550k_full_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,context512_roi256,550k,full,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
...
# Use the packaged 550k train split directly. No runtime resampling, duplicate
# filling, or difficulty redistribution is performed by this recipe.
...
if [ "${ACTUAL_TRAIN_SAMPLES}" -ne "${EXPECTED_TRAIN_SAMPLES}" ]; then
  echo "ERROR: expected ${EXPECTED_TRAIN_SAMPLES} full training samples, found ${ACTUAL_TRAIN_SAMPLES}: ${TRAIN_PATH}"
  exit 1
fi
echo "[full-train] using all ${ACTUAL_TRAIN_SAMPLES} records from ${TRAIN_PATH}"
python - "${TRAIN_PATH}" "${ACTUAL_TRAIN_SAMPLES}" > "${OUTPUT_PATH}/train_full_summary_rank${NODE_RANK}.json" <<'PY'
import json
import sys
from pathlib import Path
...
print(json.dumps({
    "mode": "full_dataset",
    "train_jsonl": str(Path(sys.argv[1]).resolve()),
...
persist_inspection_output() {
  ...
  local cloud_full_summary="${OSB_SHARE_PATH%/}/${RUN_ID}_train_full_summary_rank${NODE_RANK}.json"
  mkdir -p "$(dirname "${cloud_report}")"
  cp -f "${DATASET_INSPECTION_REPORT}" "${cloud_report}"
  if [ -f "${OUTPUT_PATH}/train_full_summary_rank${NODE_RANK}.json" ]; then
    cp -f "${OUTPUT_PATH}/train_full_summary_rank${NODE_RANK}.json" "${cloud_full_summary}"
  fi
...
echo "Train mode:   full dataset (${ACTUAL_TRAIN_SAMPLES} samples, no runtime resampling)"
echo "Eval:         ${EVAL_PATH}"
