...
# ============================================================
# NPU inference
# Fixed recipe: phase_a | lane-only centerline | dinov2 + Qwen3 text LLM | no DeepStack
...
LOCAL_OUTPUT_ROOT=${LOCAL_OUTPUT_ROOT:-${OBS_CACHE}/test_phase_a_lane_dinov2_qwen3_output_${RUN_ID}}  # Local inference output root containing json, viz, metrics, and stitched maps.
...
TRANSFORMERS_SPEC=${TRANSFORMERS_SPEC:-"transformers==4.56.2"}  # Qwen3 text support on the current NPU image.
...
if [[ "${INSTALL_DEPS}" =~ ^(1|true|True|TRUE|yes|YES)$ ]]; then
  ...
  pip install "sentencepiece>=0.1.99" "tiktoken>=0.7.0" "${TRANSFORMERS_SPEC}" "tokenizers>=0.22.0,<0.23.0"
...
export RDZV_ID=${RDZV_ID:-test_phase_a_lane_dinov2_qwen3_${RUN_ID}}
