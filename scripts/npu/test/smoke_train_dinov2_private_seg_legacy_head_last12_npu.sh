...
RUN_ID=${RUN_ID:-dinov2_private_seg_legacy_head_last12_smoke_$(date -u +%Y%m%d_%H%M%S)}
OUTPUT_DIR=${OUTPUT_DIR:-${OBS_CACHE}/outputs/${RUN_ID}}

echo "============================================================"
echo "DINOv2 private segmentation legacy-head last12 smoke"
echo "Repo:          ${REPO_ROOT}"
echo "Python:        ${PYTHON}"
...
"${PYTHON}" -m torch.distributed.run \
  ...
  --vision_unfreeze_last_n_blocks 12 \
  --num_train_epochs 10 \
  --max_steps "${MAX_STEPS}" \
