...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_b_lane_intersection_dinov2_nodeepstack}  # SwanLab group name for related runs.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_phase_b_lane_intersection_dinov2_qwen3vl8b_nodeepstack,from_stage_a}  # SwanLab experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_b,lane_intersection,dinov2,qwen3vl8b,nodeepstack,from_stage_a,unimapgen_v9}  # Comma-separated SwanLab tags for filtering runs.
...
export RDZV_ID=${RDZV_ID:-sft_phase_b_lane_intersection_dinov2_${RUN_ID}}         # Unique rendezvous id for this distributed run.
...
INIT_MODEL_PATH=$(python - "${CHECKPOINT_INPUT_PATH}" <<'PY'                      # Initial model path passed to train_qwen. Stage B uses the Stage-A checkpoint.
