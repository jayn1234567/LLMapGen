...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 true-local512 550k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 true-local512-550k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_local512_550k_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/local512/local512_550k.tar}  # Canonical true-local512 Dataset V2 package.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-local512}                                    # Top-level directory stored in local512_550k.tar.
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-550000}                          # Block training if the archive is not the finalized 550k package.
...
ENABLE_EVAL=False                                                                 # Keep eval disabled so checkpoint save does not overlap an eval memory peak.
SAVE_BEST_EVAL_LOSS=False                                                         # Eval-best checkpointing is disabled together with Trainer evaluation.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_local512_550k_original_dinov2_caprl4b}  # Group for the true-local512 run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_local512_550k_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,local512,550k,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
...
if not path.is_file():
    raise SystemExit(f"Train JSONL not found: {path}")
...
echo "Train count:  ${ACTUAL_TRAIN_SAMPLES}"
echo "Eval:         ${EVAL_PATH}"
