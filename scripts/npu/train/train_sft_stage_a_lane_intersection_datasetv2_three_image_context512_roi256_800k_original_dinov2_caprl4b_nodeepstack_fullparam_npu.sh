...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane + Pose three-image context512/ROI256 800k | original DINOv2 + CapRL-Qwen3VL-4B full parameter | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 three-image Raw-Lane+Pose context512-ROI256-800k original-DINOv2 CapRL full-parameter SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_three_image_context512_roi256_800k_original_dinov2_caprl4b_fullparam_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/context512_roi256_rawpos/context512_roi256_rawlane_pose_800k.tar}  # Final context512/ROI256 Raw-Lane + Pose three-image 800k package.
EXPECTED_DATASET_VARIANTS=${EXPECTED_DATASET_VARIANTS:-context512_roi256_rawlane_pose_800k,context512_roi256_rawlane_pose,rawlane_pose_three_image_context512_roi256_800k}  # Accept the released package identity and its historical builder aliases.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-context512_roi256_rawlane_pose_800k}        # Preferred TAR root; dynamic resolution handles the alternate released root name.
...
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-800000}                         # The OBS package is already difficulty-stratified; DI must consume it unchanged.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-512}                  # All three context inputs must decode as 512x512; labels remain central ROI256.
...
PER_DEVICE_TRAIN_BATCH_SIZE=${PER_DEVICE_TRAIN_BATCH_SIZE:-1}                     # Three visual streams plus full LLM optimizer states require a conservative micro batch.
...
LR=${LR:-2e-5}                                                                    # Full-parameter Qwen learning rate from the Jiangjihua main recipe.
MM_PROJECTOR_LR=${MM_PROJECTOR_LR:-2e-5}                                          # Full-parameter multimodal projector learning rate.
MM_VISION_TOWER_LR=${MM_VISION_TOWER_LR:-2e-5}                                    # Full-parameter DINOv2 learning rate.
...
DEEPSPEED_CONFIG=${DEEPSPEED_CONFIG:-scripts/deepspeed_zero3.json}                # ZeRO-3 partitions full-model parameters, gradients, and optimizer states.
SAVE_ON_EACH_NODE=False                                                           # Rank0 publishes the ordinary gathered full-parameter checkpoint tree.
ENABLE_EVAL=False                                                                 # Match the paired three-image recipe: train only, with no in-training eval loss.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_three_image_context512_roi256_800k_original_dinov2_caprl4b_fullparam}  # Group for this full-parameter run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_three_image_context512_roi256_800k_original_dinov2_caprl4b_fullparam_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,fullparam,zero3,phase_a,lane_intersection,datasetv2,three_image,rawlane,pose,context512_roi256,800k,dinov2,original,caprl4b,nodeepstack,lr2e-5}  # Comma-separated SwanLab tags.
...
# The OBS archive is already the final deterministic 800k selection. Validate it
...
GRADIENT_ACCUMULATION_STEPS=$(( TARGET_GLOBAL_BATCH_SIZE / MICRO_BATCH ))          # 4 nodes x 8 NPUs x 1 sample gives accumulation=4 and global batch=128.
...
echo "LLM train:    full-parameter Qwen text LLM"
echo "Projector:    full-parameter training"
echo "Distributed:  DeepSpeed ZeRO-3 (${DEEPSPEED_CONFIG}); find_unused_parameters=${DDP_FIND_UNUSED_PARAMETERS}"
...
echo "Checkpoint:   ordinary full-parameter checkpoint, steps=${SAVE_STEPS}, keep=${SAVE_TOTAL_LIMIT}, empty_cache=${MLLM_NPU_EMPTY_CACHE_BEFORE_CHECKPOINT}, skip_distributed_flos=${MLLM_SKIP_DISTRIBUTED_FLOS_ON_SAVE}"
echo "Checkpoint upload: rank0 output -> ${CLOUD_OUTPUT_PATH}"
echo "============================================================"

# Launch the full-parameter recipe with ZeRO-3 model-state partitioning.
torchrun \
  --nnodes="${NNODES}" \
...
# Only rank0 publishes the ordinary full-parameter checkpoint tree.
if [[ "${NODE_RANK}" == "0" ]]; then
  mkdir -p "$(dirname "${CLOUD_OUTPUT_PATH}")"
  if [ -e "${CLOUD_OUTPUT_PATH}" ]; then
  ...
  echo "Moving rank0 full-parameter output to cloud output: ${OUTPUT_PATH} -> ${CLOUD_OUTPUT_PATH}"
  mv "${OUTPUT_PATH}" "${CLOUD_OUTPUT_PATH}"
  MOVE_EXIT=$?
