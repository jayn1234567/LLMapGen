...
# Single-node DI-like smoke for the three-image context512/ROI256 800k recipe.
# It reuses the formal DI launcher and only overrides runtime knobs.

SCRIPT_PATH=$(readlink -f "$0")
SCRIPT_DIR=$(dirname "${SCRIPT_PATH}")
REPO_ROOT=$(readlink -f "${SCRIPT_DIR}/../../..")
FORMAL_SCRIPT="${REPO_ROOT}/scripts/npu/train/train_sft_stage_a_lane_intersection_datasetv2_three_image_context512_roi256_800k_original_dinov2_caprl4b_nodeepstack_lora_llm_npu.sh"
cd "${REPO_ROOT}"
...
RUN_ID=${RUN_ID:-datasetv2_three_image_context512_roi256_800k_dinov2_caprl4b_di_like_smoke_$(date -u +%Y%m%d_%H%M%S)}
SMOKE_ROOT=${SMOKE_ROOT:-${OBS_CACHE}/outputs/datasetv2_three_image_context512_roi256_800k_di_like_smoke}
...
echo "============================================================"
echo "Dataset V2 three-image context512/ROI256-800k original-DINOv2 + CapRL-4B LLM-LoRA DI-like smoke"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
