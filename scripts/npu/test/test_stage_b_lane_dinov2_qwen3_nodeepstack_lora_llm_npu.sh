...
# ============================================================
# NPU inference
# Fixed recipe: phase_b | lane-only centerline | DINOv2 no-DeepStack with LLM LoRA | Qwen3 text LLM
...
VISION_RECIPE=dinov2                                                              # Fixed visual architecture recipe encoded by this script name.
...
VISION_TOWER=${VISION_TOWER:-${OBS_CACHE}/checkpoints/${DINO_V2_TOWER_NAME}}      # Vision tower path passed to the model loader. Multi-vision uses a comma list.
VISION_TOWER_OBS_PATH=${VISION_TOWER_OBS_PATH:-${MODEL_OBS_PATH}/${DINO_V2_TOWER_NAME}}  # OBS path for the single vision tower used by this recipe.
...
LORA_BASE_CHECKPOINT_OBS_PATH=${LORA_BASE_CHECKPOINT_OBS_PATH:-}                  # LoRA inference: OBS root of the full base checkpoint for adapter loading.
LORA_BASE_CHECKPOINT_PATH=${LORA_BASE_CHECKPOINT_PATH:-}                          # LoRA inference: local full base checkpoint for adapter loading.
LORA_BASE_DOWNLOAD_DIR=${LORA_BASE_DOWNLOAD_DIR:-${OBS_CACHE}/lora_base_checkpoint_${RUN_ID}}  # LoRA inference: local download directory for the base checkpoint.
...
if [ -n "${LORA_BASE_CHECKPOINT_OBS_PATH}" ]; then
  python -c "import moxing as mox; mox.file.copy_parallel('${LORA_BASE_CHECKPOINT_OBS_PATH}', '${LORA_BASE_DOWNLOAD_DIR}')"
  export QWEN_BASE_MODEL_PATH=$(resolve_checkpoint "${LORA_BASE_DOWNLOAD_DIR}")
elif [ -n "${LORA_BASE_CHECKPOINT_PATH}" ]; then
  export QWEN_BASE_MODEL_PATH=$(resolve_checkpoint "${LORA_BASE_CHECKPOINT_PATH}")
elif [ -z "${QWEN_BASE_MODEL_PATH:-}" ] && [ -z "${MODEL_BASE:-}" ]; then
  echo "ERROR: Stage-B LoRA inference needs LORA_BASE_CHECKPOINT_OBS_PATH, LORA_BASE_CHECKPOINT_PATH, QWEN_BASE_MODEL_PATH, or MODEL_BASE."
  exit 1
fi
