...
# ============================================================
# NPU SFT training
# Fixed recipe: Jiangjihua v9 original 33w | latest private DINOv2 last2 + CapRL-Qwen3VL-4B | no DeepStack
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=jjh33w_latest_private_dinov2_last2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_jjh33w_private_dinov2_caprl4b}  # Group for the original-data private-DINOv2 comparison.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_jjh33w_latest_private_dinov2_last2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,jjh33w,dinov2,last2,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
