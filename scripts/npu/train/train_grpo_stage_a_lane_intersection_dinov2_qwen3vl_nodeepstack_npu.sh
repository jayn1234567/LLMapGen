...
LOCAL_OUTPUT_DIR=${LOCAL_OUTPUT_DIR:-${OBS_CACHE}/grpo_phase_a_lane_intersection_dinov2_output_${RUN_ID}}  # GRPO local output directory. The complete run is uploaded to CLOUD_OUTPUT_DIR at the end.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-grpo_phase_a_lane_intersection_dinov2_nodeepstack}  # SwanLab group name for related experiments.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-grpo_phase_a_lane_intersection_dinov2_qwen3vl8b_nodeepstack}  # SwanLab experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-grpo,phase_a,lane_intersection,dinov2,qwen3vl8b,nodeepstack,vllm-ascend}  # SwanLab comma-separated tags.
