...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane local256 stratified 200k | original DINOv2 + CapRL-Qwen3VL-4B LLM LoRA | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 Raw-Lane local256 stratified-200k original-DINOv2 CapRL LLM-LoRA SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_rawlane_local256_200k_stratified_original_dinov2_caprl4b_lora_llm_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/local256_200k_rawlane/local256_200k.tar}  # Final preselected Raw-Lane local256 200k package.
EXPECTED_DATASET_VARIANTS=${EXPECTED_DATASET_VARIANTS:-rawlane_local256_550k,rawlane_local256_200k,local256_550k,local256_200k,local256}  # Accepted metadata names; semantic preflight still requires the Raw-Lane overlay.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-local256_200k}                              # Preferred archive root; dynamic resolution handles a differently named single dataset root.
...
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-200000}                         # The OBS package is already difficulty-stratified; DI must consume it unchanged.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-256}                  # Raw image width and height in Raw-Lane local256.
...
RAW_LANE_PROMPT_TEXT=${RAW_LANE_PROMPT_TEXT:-"white lane overlay predicted by a PV camera model"}  # Required in every training/eval/test prompt.
...
PER_DEVICE_TRAIN_BATCH_SIZE=${PER_DEVICE_TRAIN_BATCH_SIZE:-4}                     # Four samples per NPU; 32 NPUs produce the requested effective batch 128 without accumulation.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_rawlane_local256_200k_stratified_original_dinov2_caprl4b_lora_llm}  # Group for this LoRA run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_rawlane_local256_200k_stratified_original_dinov2_caprl4b_lora_llm_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,llm_lora,phase_a,lane_intersection,datasetv2,rawlane,local256,200k,stratified,dinov2,original,caprl4b,nodeepstack,lr2e-4}  # Comma-separated SwanLab tags.
...
EXPECTED_NNODES=${EXPECTED_NNODES:-4}                                              # Formal recipe topology: four DI worker nodes.
EXPECTED_NPROC_PER_NODE=${EXPECTED_NPROC_PER_NODE:-8}                              # Formal recipe topology: eight NPUs per worker.
if [ "${NNODES}" -ne "${EXPECTED_NNODES}" ] || [ "${NPROC_PER_NODE}" -ne "${EXPECTED_NPROC_PER_NODE}" ]; then
  echo "ERROR: this formal LoRA recipe requires ${EXPECTED_NNODES} nodes x ${EXPECTED_NPROC_PER_NODE} NPUs; resolved nnodes=${NNODES}, nproc_per_node=${NPROC_PER_NODE}."
  exit 2
fi
if [ "${PER_DEVICE_TRAIN_BATCH_SIZE}" -ne 4 ] || [ "${TARGET_GLOBAL_BATCH_SIZE}" -ne 128 ]; then
  echo "ERROR: this formal LoRA recipe fixes per-device batch=4 and effective global batch=128."
  exit 2
fi
...
# Verify that the archive is the Raw-Lane build, not an older Dataset V2 package
# with a similar directory layout.
...
if not info_path.is_file():
    raise SystemExit(f"Raw-Lane dataset metadata not found: {info_path}")
payload = json.loads(info_path.read_text(encoding="utf-8"))
actual_variant = payload.get("dataset_variant") or payload.get("active_variant")
...
if overlay.get("raw_lane_overlay") is not True:
    raise SystemExit("dataset_info.json does not enable raw_lane_overlay")
if overlay.get("raw_lane_overlay_source") != "patch_tif/0_lane.tif":
    raise SystemExit(f"Unexpected raw lane source: {overlay.get('raw_lane_overlay_source')!r}")
print(f"[rawlane-preflight] metadata passed: variant={actual_variant} overlay={overlay}")
PY

# The OBS archive is already the final deterministic 200k selection. Validate it
# in place so DI never resamples it or changes record order.
...
print(f"[rawlane-preflight] preselected train package passed: records={records}")
PY
...
echo "Train asset:  preselected OBS package, expected_records=${EXPECTED_TRAIN_SAMPLES}, resampled_on_di=False"
echo "Eval:         ${EVAL_PATH}"
