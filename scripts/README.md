...
- `BEST_CHECKPOINT_SAVE_MODE=rotating_create_only` is the current NPU cloud default. A new best creates a unique directory under `best_candidates/` or `eval_best_candidates/`, writes metadata, then writes `_SUCCESS` last.
