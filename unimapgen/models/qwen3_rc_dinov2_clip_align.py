...
def load_visual_encoder_checkpoint(
    ...
    # 不同 Stage A checkpoint 的权重 key 前缀可能不同，这里统一做一层兼容抽取。
...
class Qwen3RCDinoClipAlignModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        # 这一段就是 Stage 1/2/3 共享的视觉桥：视觉 token -> Qwen hidden space。
    ...
    def build_aligned_visual_tokens(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        # 几何位置编码和 token_alignment 一起负责把视觉 token 拉到更适合语言模型的表示空间。
    ...
    def grouped_clip_loss(
        ...
        # 我们不是一图一文硬配对，而是 semantic_group_key 相同都视为正样本。
