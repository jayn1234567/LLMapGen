...
def _config_overrides_from_args(args) -> dict:
    return {
        ...
        "mm_use_view_type_embedding": getattr(args, "mm_use_view_type_embedding", None),
        "mm_view_type_embedding_count": getattr(args, "mm_view_type_embedding_count", None),
        "mm_view_type_embedding_init_std": getattr(args, "mm_view_type_embedding_init_std", None),
    }
...
def main():
    ...
    parser.add_argument("--mm_use_view_type_embedding", type=lambda x: str(x).lower() in ("1", "true", "yes", "on"), default=None)
    parser.add_argument("--mm_view_type_embedding_count", type=int, default=None)
    parser.add_argument("--mm_view_type_embedding_init_std", type=float, default=None)
    ...
    for idx, record in indexed_records:
        ...
        input_images = []
        context_image = record.get(args.context_image_key)
        if context_image:
            context_image_path = Path(context_image)
            if args.test_json and not context_image_path.is_absolute():
                context_image_path = Path(args.image_folder) / context_image_path
            input_images.append(Image.open(context_image_path.resolve()).convert("RGB"))
        elif args.require_context_image:
            raise ValueError(f"Missing required context image field: {args.context_image_key}")
        input_images.append(image)
        images_tensor = process_images(input_images, image_processor, model.config)
        if len(input_images) > 1 and torch.is_tensor(images_tensor):
