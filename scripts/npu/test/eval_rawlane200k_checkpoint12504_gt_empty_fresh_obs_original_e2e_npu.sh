...
CHECKPOINT_OBS_PATH=${CHECKPOINT_OBS_PATH:-obs://yw-ads-model-training-gy1/model-dev/rc-nn/rc_base_model/2026/07/29/3bf5a8001ec6433ca4ee973564c29976/output/ma-job-a782316a-32ec-4958-ae1f-44c69fdedd3f/checkpoint-12504/}
CHECKPOINT_CACHE_ROOT=${CHECKPOINT_CACHE_ROOT:-/cache/jn/checkpoints/rawlane200k_local256_checkpoint12504}

RUN_ID=${RUN_ID:-rawlane200k_checkpoint12504_gt_empty_fresh_obs_e2e_$(date +%Y%m%d_%H%M%S)}
...
echo "============================================================"
echo "RAWLANE-200K GT-EMPTY ORACLE + FRESH OBS ORIGINAL E2E"
echo "Run id:          ${RUN_ID}"
...
echo "[rawlane200k-fresh-e2e] stage 1/3: force-download and extract clean E2E data"
rm -f "${FRESH_ARCHIVE}"
python - "${E2E_DATA_OBS_PATH}" "${FRESH_ARCHIVE}" <<'PY'
import sys
import moxing as mox

print(f"[rawlane200k-fresh-e2e] download {sys.argv[1]} -> {sys.argv[2]}", flush=True)
...
echo "[rawlane200k-fresh-e2e] stage 2/3: rebuild rawlane/local256 inputs and run fresh inference"
...
echo "[rawlane200k-fresh-e2e] stage 3/3: count/suppress GT-empty patches and run original all/low/high E2E"
...
echo "============================================================"
echo "RAWLANE-200K FRESH-OBS GT-EMPTY E2E COMPLETE"
