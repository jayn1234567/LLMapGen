...
# ============================================================
# NPU SFT training
# Fixed recipe: phase_a | lane-only centerline | DINOv2+SigLIP concat fusion, no DeepStack | Qwen3-VL-derived Qwen3 LLM
...
VISION_RECIPE=dinov2_siglip_concat                                                # Fixed visual architecture recipe encoded by this script name.
MODEL_FAMILY=qwen3vl                                                              # LLM family selector: qwen3, qwen3_5, or qwen3vl.
MODEL_LABEL=qwen3vl                                                               # Short model label used in run names, logs, and output paths.
...
DINO_V2_TOWER_OBS_PATH=${DINO_V2_TOWER_OBS_PATH:-${MODEL_OBS_PATH}/${DINO_V2_TOWER_NAME}}  # OBS path for the DINOv2 tower used by this recipe.
SIGLIP_TOWER_OBS_PATH=${SIGLIP_TOWER_OBS_PATH:-${MODEL_OBS_PATH}/${SIGLIP_TOWER_NAME}}  # OBS path for the SigLIP tower used by this recipe.
...
DEFAULT_QWEN_MODEL_NAME=Qwen3-VL-8B-Instruct                                      # Default base LLM directory name for this model family.
QWEN_MODEL_NAME=${QWEN_MODEL_NAME:-${DEFAULT_QWEN_MODEL_NAME}}                    # Base Qwen/Qwen3-VL directory name under MODEL_OBS_PATH.
QWEN_MODEL_OBS_PATH=${QWEN_MODEL_OBS_PATH:-${MODEL_OBS_PATH}/${QWEN_MODEL_NAME}}  # OBS path of the base Qwen/Qwen3-VL checkpoint.
QWEN_PATH=${QWEN_PATH:-${OBS_CACHE}/checkpoints/${QWEN_MODEL_NAME}}               # Local base Qwen/Qwen3-VL path. Download is skipped when config.json exists.
...
TRANSFORMERS_SPEC=${TRANSFORMERS_SPEC:-"transformers==4.56.2"}                    # Transformers package spec; Qwen3.5 scripts may require newer versions.
TOKENIZERS_SPEC=${TOKENIZERS_SPEC:-"tokenizers>=0.22.0,<0.23.0"}                  # Tokenizers package spec aligned with TRANSFORMERS_SPEC.
...
BEST_INFER_VISION_TOWER="${MULTI_VISION_TOWERS}"                                  # Vision tower value passed to best-infer checkpoint selection.
