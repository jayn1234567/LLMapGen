...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane + Pose three-image local256 800k | original DINOv2 + CapRL-Qwen3VL-4B LLM LoRA | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 three-image Raw-Lane+Pose local256-800k original-DINOv2 CapRL LLM-LoRA SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_three_image_local256_800k_original_dinov2_caprl4b_lora_llm_$(date -u +%Y%m%d_%H%M%S)
fi
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_three_image_local256_800k_original_dinov2_caprl4b_lora_llm}  # Group for this LoRA run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_three_image_local256_800k_original_dinov2_caprl4b_lora_llm_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,llm_lora,phase_a,lane_intersection,datasetv2,three_image,rawlane,pose,local256,800k,dinov2,original,caprl4b,nodeepstack,lr2e-4}  # Comma-separated SwanLab tags.
