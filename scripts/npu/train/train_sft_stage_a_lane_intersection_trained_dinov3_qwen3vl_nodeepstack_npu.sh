...
# ============================================================
# NPU SFT training
# Fixed recipe: phase_a | lane + intersection | trained DINOv3 + Qwen3-VL-8B | no DeepStack
...
VISION_BACKBONE=trained_dinov3                                                    # Visual backbone selector used by the generic multi-vision launcher.
# Vision assets for this recipe. Scripts only download the towers declared below.
DINO_V3_TOWER_NAME=${DINO_V3_TOWER_NAME:-facebook_dinov3-vitl16-pretrain-lvd1689m}  # Base DINOv3 HF directory used to instantiate the model skeleton.
VISION_TOWER_NAME=${VISION_TOWER_NAME:-${DINO_V3_TOWER_NAME}}                     # Single vision tower directory name under MODEL_OBS_PATH.
TRAINED_DINOV3_CHECKPOINT_OBS_PATH=${TRAINED_DINOV3_CHECKPOINT_OBS_PATH:-obs://yw-ads-training-gy1/data/external/personal/h58801830/whu/jn/model/dinov3_lora.pt}  # Optional trained DINOv3 weights. Empty keeps the base tower unchanged.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_trained_dinov3_nodeepstack}  # SwanLab group name for related runs.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_phase_a_lane_intersection_trained_dinov3_qwen3vl8b_nodeepstack}  # SwanLab experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,trained_dinov3,qwen3vl8b,nodeepstack,unimapgen_v9}  # Comma-separated SwanLab tags for filtering runs.
...
export RDZV_ID=${RDZV_ID:-sft_phase_a_lane_intersection_trained_dinov3_${RUN_ID}} # Unique rendezvous id for this distributed run.
...
REQUIRED_PATHS=("${INIT_MODEL_PATH}" "${VISION_TOWER}" "${TRAIN_PATH}" "${EVAL_PATH}" "${IMAGE_FOLDER}")
if [ -n "${TRAINED_DINOV3_CHECKPOINT_OBS_PATH}" ]; then
  REQUIRED_PATHS+=("${TRAINED_DINOV3_CHECKPOINT_PATH}")
fi
...
echo "Trained DINO: ${TRAINED_DINOV3_CHECKPOINT_OBS_PATH:-off} -> ${TRAINED_DINOV3_CHECKPOINT_PATH}"
