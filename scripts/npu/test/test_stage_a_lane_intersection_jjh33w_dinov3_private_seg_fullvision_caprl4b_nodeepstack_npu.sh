...
# ============================================================
# NPU inference
# Fixed recipe: phase_a | lane + intersection | private-seg DINOv3 + CapRL-Qwen3VL-4B | no DeepStack
...
VISION_BACKBONE=dinov3_private_seg                                               # Visual backbone selector used by the generic multi-vision launcher.
# Vision assets for this recipe. Scripts only download the towers declared below.
VISION_TOWER_NAME=facebook_dinov3-vitl16-pretrain-lvd1689m                       # Base HF DINOv3 checkpoint directory name under MODEL_OBS_PATH.
MM_VISION_TOWER_TYPE=dinov3_private_seg                                           # Model-side vision tower type: dinov2, dinov3, dinov3_private_seg, multi_moe, or multi_concat.
INPUT_IMAGE_SIZE=512                                                              # 256x256 BEV patches are resized to 512 before DINOv3 patch16 encoding.
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-gy1/data/external/personal/h58801830/whu/jjh/data/data_lane_intersection_samples_norm_33w_empty_patch.zip}  # Jiangjihua v9 best lane+intersection norm1000 dataset.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-data_lane_intersection_samples_norm_33w_empty_patch}  # Dataset directory name expected after the zip is extracted.

CHECKPOINT_OBS_LIST=${CHECKPOINT_OBS_LIST:-obs://yw-ads-model-training-gy1/model-dev/rc-nn/rc_base_model/2026/07/16/c45abe4c8dd44478bc736d0103af8930/output/ma-job-45d5a70d-e785-4e28-a37f-2b54b3884914/checkpoint-21488/}  # Comma, semicolon, or newline separated OBS checkpoint roots to evaluate.
...
VISION_TOWER_OBS_PATH=${VISION_TOWER_OBS_PATH:-${MODEL_OBS_PATH}/${VISION_TOWER_NAME}}  # OBS base DINOv3 model directory.
VISION_TOWER_CHECKPOINT_OBS_PATH=${VISION_TOWER_CHECKPOINT_OBS_PATH:-obs://yw-ads-training-gy1/data/external/personal/h58801830/whu/jn/model/dinov3_lora.pt}  # Scripted private segmentation DINOv3 checkpoint.
VISION_TOWER_CHECKPOINT=${VISION_TOWER_CHECKPOINT:-${OBS_CACHE}/checkpoints/dinov3_lora.pt}  # Local private segmentation DINOv3 checkpoint.
...
LOCAL_OUTPUT_ROOT=${LOCAL_OUTPUT_ROOT:-${OBS_CACHE}/test_phase_a_lane_intersection_dinov3_private_seg_caprl4b_output_${RUN_ID}}  # Per-run local inference output root.
...
export RDZV_ID=${RDZV_ID:-test_phase_a_lane_intersection_dinov3_private_seg_caprl4b_${RUN_ID}}  # Unique rendezvous id for this distributed run.
...
if [ -n "${VISION_TOWER_CHECKPOINT_OBS_PATH}" ]; then
  mkdir -p "$(dirname "${VISION_TOWER_CHECKPOINT}")"
  python -c "import moxing as mox; mox.file.copy('${VISION_TOWER_CHECKPOINT_OBS_PATH}', '${VISION_TOWER_CHECKPOINT}')"
fi
