...
# Formal DI recipe for full-parameter private-road DINOv2-L segmentation.
...
echo "[di-entry] reached formal DINOv2 private segmentation training script"
...
VISION_LEARNING_RATE=${VISION_LEARNING_RATE:-5e-6}
...
DICE_LOSS_WEIGHT=${DICE_LOSS_WEIGHT:-0.5}
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=dinov2_private_seg_fullft_$(date -u +%Y%m%d_%H%M%S)
fi
RUN_ID=${RUN_ID:-${DEFAULT_RUN_ID}}
LOCAL_OUTPUT_ROOT=${LOCAL_OUTPUT_ROOT:-/cache/local_model_save_path}
OUTPUT_DIR=${OUTPUT_DIR:-${LOCAL_OUTPUT_ROOT}/${RUN_ID}}
if [ -z "${OUTPUT_URL:-}" ]; then
  OUTPUT_URL=obs://yw-ads-training-gy1/data/external/personal/h58801830/whu/jn/model/dinov2_private_seg_fullft
fi
...
echo "============================================================"
echo "Formal DINOv2 private segmentation training"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
...
PY
  ...
  echo "============================================================"
  echo "Formal DI DINOv2 segmentation training PASSED"
