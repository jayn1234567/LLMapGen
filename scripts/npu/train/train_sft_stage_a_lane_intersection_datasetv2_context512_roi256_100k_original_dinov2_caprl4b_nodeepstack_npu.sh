...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 context512_roi256 100k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 context512_roi256-100k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_context512_roi256_100k_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_ROOT=${DATASET_OBS_ROOT:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data}  # OBS parent that contains the Dataset V2 context512_roi256 100k asset.
DATASET_OBS_PATH=${DATASET_OBS_PATH:-}                                           # Optional exact tar URI; empty probes common locations below DATASET_OBS_ROOT.
DATASET_OBS_ARCHIVE_NAME=${DATASET_OBS_ARCHIVE_NAME:-context512_roi256_100k.tar}           # Archive produced by the 100k Dataset V2 packaging workflow.
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-100000}                         # Hard guard against accidentally training on the 550k package.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-context512_roi256}                                    # Top-level directory stored in context512_roi256.tar.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-512}                  # Raw image width and height in Dataset V2 context512_roi256.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_context512_roi256_100k_original_dinov2_caprl4b}  # Group for the original-DINOv2 rerun.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_context512_roi256_100k_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,context512_roi256,100k,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
...
if explicit:
    ...
    archive_candidates.extend([
        f"{root}/{archive_name}",
        f"{root}/context512_roi256_100k/{archive_name}",
        f"{root}/packages/{archive_name}",
        f"{root}/output_100k/packages/{archive_name}",
        f"{root}/output_100k/{archive_name}",
        f"{root}/context512_roi256/{archive_name}",
        f"{root}/context512_roi256.tar",
        f"{root}/packages/context512_roi256.tar",
        f"{root}/output_100k/packages/context512_roi256.tar",
    ])
    directory_candidates.extend([
        f"{root}/context512_roi256_100k",
        f"{root}/context512_roi256_100k/context512_roi256",
        f"{root}/context512_roi256",
        f"{root}/output_100k/context512_roi256",
        f"{root}/output/context512_roi256",
    ])
...
if selected is None:
    ...
    raise FileNotFoundError(
        "Dataset V2 context512_roi256 100k asset was not found.\n"
...
PY
  then
    echo "ERROR: failed to resolve or download Dataset V2 context512_roi256 100k asset."
    exit 1
  fi
...
ACTUAL_TRAIN_SAMPLES=$(python - "${TRAIN_PATH}" <<'PY'
import sys
from pathlib import Path
...
if [ "${EXPECTED_TRAIN_SAMPLES}" -gt 0 ] && [ "${ACTUAL_TRAIN_SAMPLES}" -ne "${EXPECTED_TRAIN_SAMPLES}" ]; then
  echo "ERROR: expected ${EXPECTED_TRAIN_SAMPLES} train samples, found ${ACTUAL_TRAIN_SAMPLES}: ${TRAIN_PATH}"
  echo "Set DATASET_OBS_PATH to the exact 100k archive instead of continuing with the wrong dataset."
  exit 1
fi
echo "[dataset-preflight] train samples: ${ACTUAL_TRAIN_SAMPLES}"
python - "${TRAIN_PATH}" "${EVAL_PATH}" "${TEST_PATH}" <<'PY'
import json
import sys
from pathlib import Path

expected_view = "context512_roi256"
expected_roi = [128, 128, 384, 384]
expected_prompt_fragments = (
    "The input is a 512x512 context image centered on the target region.",
    "central 256x256 target ROI [128,128,384,384)",
    "relative to the target ROI, not the full context image",
)
...
for path_text in sys.argv[1:]:
    path = Path(path_text)
    if not path.is_file():
        raise FileNotFoundError(f"context dataset split is missing: {path}")
    with path.open("r", encoding="utf-8-sig") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            actual = {
                ...
                "context_image_size": meta.get("context_image_size"),
                "target_roi_in_image": meta.get("target_roi_in_image"),
            }
            expected = {
                "view_mode": expected_view,
                "target_size": 256,
                "context_image_size": 512,
                "target_roi_in_image": expected_roi,
            }
            if actual != expected:
                raise ValueError(
                    f"context512_roi256 metadata mismatch at {path}:{line_number}: "
                    f"expected={expected}, actual={actual}"
                )
            conversations = record.get("conversations") or []
            prompt = str(conversations[0].get("value", "")) if conversations else ""
            missing = [fragment for fragment in expected_prompt_fragments if fragment not in prompt]
            if missing:
                raise ValueError(
                    f"context512_roi256 prompt mismatch at {path}:{line_number}; missing={missing}"
                )
            checked += 1
print(f"[dataset-preflight] context512_roi256 metadata/prompt records passed: {checked}")
PY
CONTEXT_AUDIT_EXIT=$?
if [ "${CONTEXT_AUDIT_EXIT}" -ne 0 ]; then
  echo "[dataset-preflight] context512_roi256 metadata/prompt audit failed with exit code ${CONTEXT_AUDIT_EXIT}."
  exit "${CONTEXT_AUDIT_EXIT}"
fi
...
echo "Dataset OBS:  ${RESOLVED_DATASET_OBS_PATH}"
