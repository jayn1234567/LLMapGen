...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane context512_roi256 stratified 200k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 Raw-Lane context512_roi256 stratified-200k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_rawlane_context512_roi256_200k_stratified_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/context512_roi256_200k_rawlane/context512_roi256_200k.tar}  # Final preselected Raw-Lane context512/ROI256 200k package.
EXPECTED_DATASET_VARIANTS=${EXPECTED_DATASET_VARIANTS:-rawlane_context512_roi256_550k,rawlane_context512_roi256_200k,context512_roi256_550k,context512_roi256_200k,context512_roi256}  # Accepted metadata names; semantic preflight still requires the Raw-Lane overlay.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-context512_roi256_200k}                     # Preferred archive root; dynamic resolution handles a differently named single dataset root.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-512}                  # Raw image width and height in Raw-Lane context512_roi256.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_rawlane_context512_roi256_200k_stratified_original_dinov2_caprl4b}  # Group for this exact-ratio subset run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_rawlane_context512_roi256_200k_stratified_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,rawlane,context512_roi256,200k,stratified,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
