...
# Stage-B SFT: OBS path of the trained Stage-A checkpoint or output dir.
STAGE_A_CHECKPOINT_OBS_PATH=${STAGE_A_CHECKPOINT_OBS_PATH:-}
# Stage-B SFT: local Stage-A checkpoint path; used when OBS path is empty.
STAGE_A_CHECKPOINT_PATH=${STAGE_A_CHECKPOINT_PATH:-}
# Local download directory for the Stage-A checkpoint.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_b_lane_intersection_dinov2_nodeepstack}
# SwanLab experiment display name.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_phase_b_lane_intersection_dinov2_qwen3vl8b_nodeepstack,from_stage_a}
# SwanLab comma-separated tags.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_b,lane_intersection,dinov2,qwen3vl8b,nodeepstack,from_stage_a}
...
export RDZV_ID=${RDZV_ID:-sft_phase_b_lane_intersection_dinov2_${RUN_ID}}
