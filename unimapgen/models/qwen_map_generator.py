...
class QwenSatelliteMapGenerator(nn.Module):
    ...
    def encode_prefix(
        ...
        prefix_embeds = torch.cat([sat_tokens, prompt_embeds, state_embeds], dim=1)
        prefix_mask = torch.cat([sat_mask, prompt_mask, state_mask], dim=1)
    ...
    def generate(
        ...
        for step in range(int(max_new_tokens)):
            ...
            if int(top_k) > 1:
                ...
                next_tok = allowed[idxs.gather(1, pick)]
            else:
                next_tok = allowed[restricted.argmax(dim=-1, keepdim=True)]
