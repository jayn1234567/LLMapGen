...
# Single-node DI-like smoke for matched-200k experiment B.
# It reuses the formal DI launcher and only overrides runtime knobs.

SCRIPT_PATH=$(readlink -f "$0")
SCRIPT_DIR=$(dirname "${SCRIPT_PATH}")
REPO_ROOT=$(readlink -f "${SCRIPT_DIR}/../../..")
FORMAL_SCRIPT="${REPO_ROOT}/scripts/qwen3vl_native/train/train_sft_stage_a_lane_intersection_datasetv2_three_image_context512_roi256_200k_qwen3vl8b_lora_npu.sh"
cd "${REPO_ROOT}"
...
RUN_ID=${RUN_ID:-datasetv2_three_image_context512_roi256_200k_native_qwen3vl8b_di_like_smoke_$(date -u +%Y%m%d_%H%M%S)}
SMOKE_ROOT=${SMOKE_ROOT:-${OBS_CACHE}/outputs/datasetv2_three_image_context512_roi256_200k_native_qwen3vl8b_di_like_smoke}
...
echo "============================================================"
echo "Experiment B: matched-200k three-image context512/ROI256 native Qwen3-VL-8B LoRA smoke"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
