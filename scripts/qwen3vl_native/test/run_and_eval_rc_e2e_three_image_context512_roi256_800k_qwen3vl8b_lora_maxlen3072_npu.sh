...
# Native Qwen3-VL-8B three-image context512/center-ROI256 maxlen3072 E2E
# inference and original RC all/low/high plus intersection evaluation.
# CHECKPOINT_OBS_PATH is intentionally required so one launcher can evaluate
# any checkpoint from the completed training run without embedding stale OBS IDs.
...
: "${CHECKPOINT_OBS_PATH:?Set CHECKPOINT_OBS_PATH to the context512_roi256 800k three-image native Qwen3-VL-8B LoRA maxlen3072 checkpoint OBS directory}"
CHECKPOINT_NAME=${CHECKPOINT_NAME:-$(basename "${CHECKPOINT_OBS_PATH%/}")}
CHECKPOINT_CACHE_ROOT=${CHECKPOINT_CACHE_ROOT:-/cache/jn/checkpoints/native_qwen3vl_three_image_context512_roi256_800k_maxlen3072}
...
RUN_ID=${RUN_ID:-three_image_context512_roi256_800k_native_qwen3vl8b_lora_maxlen3072_${CHECKPOINT_NAME}_e2e_$(date -u +%Y%m%d_%H%M%S)}
...
INFERENCE_DATASET_ROOT=${INFERENCE_DATASET_ROOT:-${RUN_WORK_ROOT}/three_image_context512_roi256_dataset}
...
echo "============================================================"
echo "NATIVE QWEN3-VL-8B THREE-IMAGE CONTEXT512/ROI256 800K MAXLEN3072 E2E"
...
echo "View/coordinates:   context512_roi256 / center ROI256 norm1000"
...
echo "[native-three-image-e2e] stage 2/7: build aligned context512/ROI256 image triplets"
...
if (
    summary.get("view_mode") != "context512_roi256"
    or summary.get("patch_size") != 256
    or summary.get("context_size") != 512
    or summary.get("target_roi_in_image") != [128, 128, 384, 384]
):
...
echo "============================================================"
echo "NATIVE QWEN3-VL THREE-IMAGE CONTEXT512/ROI256 MAXLEN3072 E2E COMPLETE"
