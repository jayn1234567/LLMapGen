...
# ============================================================
# NPU SFT training
# Fixed recipe: phase_a | lane + intersection | dinov3 + Qwen3-VL-8B | no DeepStack | LLM LoRA
...
QWEN_PATH=${QWEN_PATH:-${OBS_CACHE}/checkpoints/Qwen3-VL-8B-Instruct}           # Local Qwen3-VL base model path used by Stage-A SFT.
...
LR=${LR:-1e-4}                                                                # Base learning rate for LLM LoRA adapters and default trainable parameters.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_dinov3_nodeepstack_lora_llm}  # SwanLab group name for related experiments.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_phase_a_lane_intersection_dinov3_qwen3vl8b_nodeepstack_lora_llm}  # SwanLab experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,dinov3,qwen3vl8b,nodeepstack,lora,llm_lora}  # SwanLab comma-separated tags.
...
export RDZV_ID=${RDZV_ID:-sft_phase_a_lane_intersection_dinov3_lora_llm_${RUN_ID}}
