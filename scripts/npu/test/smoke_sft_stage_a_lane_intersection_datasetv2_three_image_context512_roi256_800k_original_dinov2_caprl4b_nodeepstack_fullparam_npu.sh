...
# Single-node DI-like smoke for the three-image context512/ROI256 800k full-parameter recipe.
# It reuses the formal DI launcher and only overrides runtime knobs.

SCRIPT_PATH=$(readlink -f "$0")
SCRIPT_DIR=$(dirname "${SCRIPT_PATH}")
REPO_ROOT=$(readlink -f "${SCRIPT_DIR}/../../..")
FORMAL_SCRIPT="${REPO_ROOT}/scripts/npu/train/train_sft_stage_a_lane_intersection_datasetv2_three_image_context512_roi256_800k_original_dinov2_caprl4b_nodeepstack_fullparam_npu.sh"
cd "${REPO_ROOT}"
...
RUN_ID=${RUN_ID:-datasetv2_three_image_context512_roi256_800k_dinov2_caprl4b_fullparam_di_like_smoke_$(date -u +%Y%m%d_%H%M%S)}
SMOKE_ROOT=${SMOKE_ROOT:-${OBS_CACHE}/outputs/datasetv2_three_image_context512_roi256_800k_fullparam_di_like_smoke}
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/context512_roi256_rawpos/context512_roi256_rawlane_pose_800k.tar}
DATASET_ARCHIVE_PATH=${DATASET_ARCHIVE_PATH:-${OBS_CACHE}/datasets/context512_roi256_rawlane_pose_800k.tar}
DATASET_EXTRACT_ROOT=${DATASET_EXTRACT_ROOT:-${OBS_CACHE}/datasets/context512_roi256_rawlane_pose_800k_extract}
DATASET_PATH=${DATASET_PATH:-${DATASET_EXTRACT_ROOT}/context512_roi256_rawlane_pose_800k}
...
echo "============================================================"
echo "Dataset V2 three-image context512/ROI256-800k original-DINOv2 + CapRL-4B full-parameter DI-like smoke"
echo "Repo:             ${REPO_ROOT}"
echo "Python:           ${PYTHON}"
...
echo "DINO input:       518"
echo "Distributed:      DeepSpeed ZeRO-3 full-parameter SFT"
...
if ! find "${CHECKPOINT_DIR}" -maxdepth 2 -type f \( \
  -name 'model.safetensors' -o -name 'model.safetensors.index.json' -o \
  -name 'pytorch_model.bin' -o -name 'pytorch_model.bin.index.json' \
\) -print -quit | grep -q .; then
  echo "ERROR: gathered full-parameter model weights are missing below ${CHECKPOINT_DIR}" >&2
  exit 1
fi
...
echo "Training log:   ${TRAIN_LOG}"
echo "NPU memory log: ${NPU_MEMORY_LOG}"
echo "Formal DI keeps the same ZeRO-3 full-parameter path and derives accumulation from world size."
