...
QWEN_MODEL_NAME=${QWEN_MODEL_NAME:-Qwen3.5-4B-Instruct}                         # Qwen3.5 text LLM directory name under MODEL_OBS_PATH. Override if your checkpoint uses another name.
QWEN_MODEL_OBS_PATH=${QWEN_MODEL_OBS_PATH:-${MODEL_OBS_PATH}/${QWEN_MODEL_NAME}}  # OBS path for the Qwen3.5 text LLM base checkpoint.
QWEN_PATH=${QWEN_PATH:-${OBS_CACHE}/checkpoints/${QWEN_MODEL_NAME}}             # Local Qwen3.5 text LLM base model path used by Stage-A SFT.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_dinov2_qwen3_5_nodeepstack}     # SwanLab group name for related experiments.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_phase_a_lane_dinov2_qwen3_5_nodeepstack}  # SwanLab experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane,dinov2,qwen3_5,nodeepstack,unimapgen_v9}  # SwanLab comma-separated tags.
...
export RDZV_ID=${RDZV_ID:-sft_phase_a_lane_dinov2_qwen3_5_${RUN_ID}}
