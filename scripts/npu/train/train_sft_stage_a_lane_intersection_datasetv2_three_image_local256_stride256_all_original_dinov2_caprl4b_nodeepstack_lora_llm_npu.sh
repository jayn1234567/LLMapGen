...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane + Pose three-image local256 stride-256-all | original DINOv2 + CapRL-Qwen3VL-4B LLM LoRA | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 three-image Raw-Lane+Pose local256 stride256-all original-DINOv2 CapRL LLM-LoRA SFT launcher"
...
VISION_TOWER_NAME=facebook_dinov2-large                                           # Original DINOv2 checkpoint directory name under MODEL_OBS_PATH.
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_three_image_local256_stride256_all_original_dinov2_caprl4b_lora_llm_$(date -u +%Y%m%d_%H%M%S)
fi
...
DATASET_OBS_PATH=${DATASET_OBS_PATH:-obs://yw-ads-training-2-gy1/data/external/personal/h58801830/jn/data/context512_roi256_three_image/rawlane_pose_three_image_local256_stride256_all.tar}  # Final local256 Raw-Lane + Pose three-image package.
EXPECTED_DATASET_VARIANTS=${EXPECTED_DATASET_VARIANTS:-local256_rawlane_pose_800k,local256_rawlane_pose,rawlane_pose_three_image_local256_800k,rawlane_pose_three_image_local256_stride256_all,rc_dataset_v2_three_image_local256_stride256_all_v1}  # Accept the released package identity and its historical builder aliases.
DATASET_DIR_NAME=${DATASET_DIR_NAME:-rawlane_pose_three_image_local256_stride256_all}  # Preferred TAR root; dynamic resolution handles an alternate top-level name.
...
EXPECTED_TRAIN_SAMPLES=${EXPECTED_TRAIN_SAMPLES:-0}                             # 0 means use the package's actual train count; a positive value enables an explicit count check.
...
DATASET_EXPECTED_IMAGE_SIZE=${DATASET_EXPECTED_IMAGE_SIZE:-256}                  # All three local256 inputs must decode as 256x256.
DATASET_COORD_MAX=${DATASET_COORD_MAX:-1000}                                     # Expected norm1000 target-coordinate upper bound.
DATASET_ALLOWED_TARGET_LANE_TYPES=${DATASET_ALLOWED_TARGET_LANE_TYPES:-"common right_turn waiting_area bus_lane main_auxiliary_connector other"}  # Final six-way Raw-Lane taxonomy.
...
RAW_LANE_PROMPT_TEXT=${RAW_LANE_PROMPT_TEXT:-"second image is a lane image predicted by a PV camera model"}  # Required separate Raw-Lane role description.
POSE_PROMPT_TEXT=${POSE_PROMPT_TEXT:-"third image is a historical vehicle-trajectory image"}  # Required separate Pose role description.
...
MM_PROJECTOR_LR=${MM_PROJECTOR_LR:-2e-4}                                          # Full-parameter multimodal projector learning rate.
MM_VISION_TOWER_LR=${MM_VISION_TOWER_LR:-2e-5}                                    # Train DINOv2 at one tenth of the LoRA/projector rate for stability.
LORA_ENABLE=${LORA_ENABLE:-True}                                                   # Enable PEFT LoRA for the Qwen text LLM.
LORA_TARGET_SCOPE=${LORA_TARGET_SCOPE:-llm}                                       # Keep LoRA inside the text LLM; vision/projector use ordinary trainable parameters.
LORA_R=${LORA_R:-8}                                                               # LoRA rank used by the existing Jiangjihua-route LoRA recipes.
...
MM_VISION_SELECT_LAYER=${MM_VISION_SELECT_LAYER:--2}                              # Jiangjihua original DINOv2 recipe uses the penultimate selected visual layer.
COORDINATE_TOKEN_MODE=${COORDINATE_TOKEN_MODE:-none}                              # none for ordinary numeric JSON; angle enables <0>...<1000> tokens.
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_three_image_local256_stride256_all_original_dinov2_caprl4b_lora_llm}  # Group for this LoRA run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_three_image_local256_stride256_all_original_dinov2_caprl4b_lora_llm_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,lora,llm_lora,phase_a,lane_intersection,datasetv2,three_image,rawlane,pose,local256,stride256_all,dinov2,original,caprl4b,nodeepstack,lr2e-4}  # Comma-separated SwanLab tags.
...
export MLLM_LOG_MULTI_IMAGE_SHAPE=${MLLM_LOG_MULTI_IMAGE_SHAPE:-1}                # Prove that each sample reaches the vision tower as three ordered images.
...
export MLLM_SKIP_DISTRIBUTED_FLOS_ON_SAVE=${MLLM_SKIP_DISTRIBUTED_FLOS_ON_SAVE:-False} # Preserve the ordinary Trainer save sequence after pre-save cleanup.
...
# Verify the packaged three-image metadata and the completed builder validation.
python - "${DATASET_PATH}" "${EXPECTED_DATASET_VARIANTS}" <<'PY'
import json
import sys
from pathlib import Path
...
actual_variant = (
    payload.get("dataset_variant")
    or payload.get("active_variant")
    or payload.get("dataset_version")
)
if actual_variant not in expected_variants:
    raise SystemExit(f"Expected one of dataset variants/versions {sorted(expected_variants)!r}, found {actual_variant!r}")
overlay = payload.get("input_overlay") or {}
multi = payload.get("multi_image_input") or {}
...
accepted_prompt_contracts = {
    "three_image_roles_concise_v2",
    "three_image_local256_stride256_v1",
}
...
if metadata_prompt_contract not in (None, "") and metadata_prompt_contract not in accepted_prompt_contracts:
    raise SystemExit(
        "Unexpected three-image prompt contract: "
        f"{metadata_prompt_contract!r}; "
        f"expected one of {sorted(accepted_prompt_contracts)!r}"
    )
...
if validation_prompt_contract not in (None, "") and validation_prompt_contract not in accepted_prompt_contracts:
    raise SystemExit(
        "Builder validation uses an unsupported prompt contract: "
        f"{validation_prompt_contract!r}; expected one of {sorted(accepted_prompt_contracts)!r}"
    )
...
# The OBS archive is already the final deterministic selection. Validate it in
# place so DI never resamples it or changes record order.
TRAIN_DATASET_SUMMARY=${TRAIN_DATASET_SUMMARY:-${OUTPUT_PATH}/train_dataset_summary_rank${NODE_RANK}.json}
python - "${TRAIN_PATH}" "${EXPECTED_TRAIN_SAMPLES}" "${TRAIN_DATASET_SUMMARY}" <<'PY'
import json
import sys
from pathlib import Path
...
records = 0
for line_number, line in enumerate(train_path.open("r", encoding="utf-8"), 1):
    if not line.strip():
        continue
    try:
        json.loads(line)
    except json.JSONDecodeError as exc:
        raise SystemExit(f"Invalid JSON at {train_path}:{line_number}: {exc}") from exc
    records += 1
if expected > 0 and records != expected:
    raise SystemExit(f"Expected {expected} preselected train records, found {records} in {train_path}")
payload = {
    "dataset_obs_path": "preselected_by_launcher",
    "train_path": str(train_path),
    "train_records": records,
    "resampled_on_di": False,
}
summary_path.parent.mkdir(parents=True, exist_ok=True)
summary_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
if expected > 0:
    print(f"[three-image-preflight] preselected train package passed: records={records}")
else:
    print(f"[three-image-preflight] package train count accepted from archive: records={records}")
PY
...
if [ $(( TARGET_GLOBAL_BATCH_SIZE % MICRO_BATCH )) -ne 0 ]; then
  echo "ERROR: TARGET_GLOBAL_BATCH_SIZE=${TARGET_GLOBAL_BATCH_SIZE} is not divisible by total micro-batch ${MICRO_BATCH}."
  exit 2
fi
GRADIENT_ACCUMULATION_STEPS=$(( TARGET_GLOBAL_BATCH_SIZE / MICRO_BATCH ))          # 4 nodes x 8 NPUs x 4 samples gives accumulation=1 and global batch=128.
...
echo "============================================================"
echo "Recipe:       ${DATASET_PHASE} | ${MAP_TASK} | ${VISION_BACKBONE} | local256 stride256-all"
echo "Init model:   ${INIT_MODEL_PATH}"
echo "Vision tower: ${VISION_TOWER}"
echo "Vision OBS:   ${MODEL_OBS_PATH}/${VISION_TOWER_NAME}"
...
echo "Train asset:  preselected OBS package, expected_records=${EXPECTED_TRAIN_SAMPLES:-actual}, resampled_on_di=False"
echo "Eval:         ${EVAL_PATH}"
...
# Launch the recipe entrypoint. LoRA keeps Qwen memory bounded without DeepSpeed.
torchrun \
  --nnodes="${NNODES}" \
...
# Only rank0 publishes the ordinary LoRA checkpoint tree. Each checkpoint stores
# the Qwen adapter plus full projector/vision parameters in non_lora_trainables.bin.
if [[ "${NODE_RANK}" == "0" ]]; then
  mkdir -p "$(dirname "${CLOUD_OUTPUT_PATH}")"
  if [ -e "${CLOUD_OUTPUT_PATH}" ]; then
  ...
  echo "Moving rank0 LoRA output to cloud output: ${OUTPUT_PATH} -> ${CLOUD_OUTPUT_PATH}"
  mv "${OUTPUT_PATH}" "${CLOUD_OUTPUT_PATH}"
  MOVE_EXIT=$?
