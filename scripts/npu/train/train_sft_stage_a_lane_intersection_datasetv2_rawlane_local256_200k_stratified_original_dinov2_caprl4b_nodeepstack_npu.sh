...
# ============================================================
# NPU SFT training
# Fixed recipe: Dataset V2 Raw-Lane local256 stratified 200k | original DINOv2 + CapRL-Qwen3VL-4B | no DeepStack
# This file is self-contained and does not call another project .sh file.
# ============================================================

echo "[di-entry] reached Dataset V2 Raw-Lane local256 stratified-200k original-DINOv2 CapRL SFT launcher"
...
if [ -n "${MA_VJ_NAME:-}" ]; then
  ...
  DEFAULT_RUN_ID=datasetv2_rawlane_local256_200k_stratified_original_dinov2_caprl4b_$(date -u +%Y%m%d_%H%M%S)
fi
...
SWANLAB_GROUP=${SWANLAB_GROUP:-sft_phase_a_lane_intersection_datasetv2_rawlane_local256_200k_stratified_original_dinov2_caprl4b}  # Group for this exact-ratio subset run.
SWANLAB_EXPERIMENT_NAME=${SWANLAB_EXPERIMENT_NAME:-sft_datasetv2_rawlane_local256_200k_stratified_original_dinov2_caprl4b_ep8}  # Experiment display name.
SWANLAB_TAGS=${SWANLAB_TAGS:-sft,phase_a,lane_intersection,datasetv2,rawlane,local256,200k,stratified,dinov2,original,caprl4b,nodeepstack}  # Comma-separated SwanLab tags.
